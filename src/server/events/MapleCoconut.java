...
public class MapleCoconut extends MapleEvent {
	...
	super.onMapLoad(chr);
        chr.getClient().write(MaplePacketCreator.coconutScore(getCoconutScore()));
    }
                ...
                if (getMapleScore() == getStoryScore()) {
                    ...
                    for (MapleCharacter chr : getMap(0).getCharactersThreadsafe()) {
                        if (chr.getTeam() == (getMapleScore() > getStoryScore() ? 0 : 1)) {
                            chr.getClient().write(MaplePacketCreator.showEffect("event/coconut/victory"));
                            chr.getClient().write(MaplePacketCreator.playSound("Coconut/Victory"));
                        } else {
                            chr.getClient().write(MaplePacketCreator.showEffect("event/coconut/lose"));
                            chr.getClient().write(MaplePacketCreator.playSound("Coconut/Failed"));
                        }
                    }
                    warpOut();
                }
