...
public class PokemonBattle {
    ...
    public void updateHP(int i) {
        if (battlers[i] != null && battlers[i].getMonster() != null && characterIds[i == 0 ? 1 : 0] > 0) {
            ...
            if (chrr != null) {
                chrr.getClient().write(MobPacket.showBossHP(9300184, battlers[i].getCurrentHP(), battlers[i].calcHP()));
            }
        }
    }
    ...
    public void initiate(MapleCharacter initiator, PokemonMap mapp) { //wild battle
        ...
        initiator.getClient().write(MaplePacketCreator.musicChange(wildBGM));
    ...
    public void initiate(MapleCharacter initiator, MapleNPC npc) { //wild battle
        ...
        initiator.dropMessage(-6, "What will you do? @use @info @ball @run @battlehelp");
        turn[1] = makeRandomTurn();
        initiator.getClient().write(MaplePacketCreator.musicChange(npcBGM));
        firstTurn(battlers[0], battlers[1]);
    }
    ...
    public void initiate() { //trainer battle
        ...
        initiator.getClient().write(MaplePacketCreator.musicChange(trainerBGM));
        initiator2.getClient().write(MaplePacketCreator.musicChange(trainerBGM));
        firstTurn(battlers[0], battlers[1]);
                    ...
                    EtcTimer.getInstance().schedule(new Runnable() {
                        ...
                        public void run() {
                            if (user != null && !disposed) {
                                user.getClient().write(MaplePacketCreator.playSound("Cokeplay/Fall"));
                                catchEffect(battlers[1].getMonster());
                            } else {
                                dispose();
                            }
                    ...
                    EtcTimer.getInstance().schedule(new Runnable() {
                        ...
                        public void run() {
                            if (user != null && !disposed) {
                                ...
                                user.getClient().write(MaplePacketCreator.playSound("Romio/discovery"));
                    ...
                    public void run() {
                        ...
                        switch (turn[i]) {
                            ...
                            case ATTACK:
                                ...
                                final List<ILifeMovementFragment> moves = new ArrayList<ILifeMovementFragment>();
/*                                if (theAtk == null) { //simple walk to monster -> atk -> walk back
                                    ...
                                    if (shouldAdd) { //walk up to the required range of the attack
                                        ...
                                        map.broadcastMessage(MobPacket.moveMonster(false, (byte) -1, 0, 0, 0, 0, battlers[i].getMonster().getObjectId(), startPos, startPos, moves2));
                                    }
                                ...
                                if (mPos == null) {
//                                    map.broadcastMessage(MobPacket.moveMonster(skillByte > 0, skillByte, 0, 0, 0, 0, battlers[i].getMonster().getObjectId(), startPos, startPos, moves));
                                    moves.clear();
                                }
                                EtcTimer.getInstance().schedule(new Runnable() {
                                    ...
                                    public void run() {
                                        ...
                                        if (mPos != null) {
//                                            map.broadcastMessage(MobPacket.moveMonster(skillByte > 0, skillByte, 0, 0, 0, 0, battlers[i].getMonster().getObjectId(), mPos, mPos, moves));
                                            moves.clear();
                                        }
...
/*                                            if (basedamage > 1) {
                                                ...
                                                map.broadcastMessage(MobPacket.moveMonster(true, skillByte2, 0, 0, 0, 0, battlers[i == 1 ? 0 : 1].getMonster().getObjectId(), battlers[i == 1 ? 0 : 1].getMonster().getTruePosition(), battlers[i == 1 ? 0 : 1].getMonster().getTruePosition(), moves));
                                            }*/
                                        ...
                                        if (battlers[i == 1 ? 0 : 1].getCurrentHP() <= 0) {
                                                        ...
                                                        if (b != null && b.getMonsterId() == z) {
                                                            ...
                                                            if (b.getTrueLevel() > oLevel) {
                                                                ...
                                                                if (!b.getStats().getName().equals(oName)) {
                                                                    ch.getClient().write(MaplePacketCreator.playSound("5th_Maple/prize"));
                                            ...
                                            if (instanceid >= 0 && i == 0 && npcTeam == null) {
                                                ...
                                                chr.getClient().write(MaplePacketCreator.playSound("Romio/discovery"));
                                                dispose(chr, false);
                                                return;
                                            }
                                            if (instanceid >= 0 && i == 0 && npcTeam != null) {
