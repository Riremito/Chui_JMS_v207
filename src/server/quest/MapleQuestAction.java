    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
                ...
                for (QuestItem item : items) {
                    ...
                    if (count < 0) { // remove items
                        ...
                        if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                            final String msg1 = "勲章 - " + name + "を獲得しました！";
            ...
            case nextQuest:
                ...
                c.getClient().write(MaplePacketCreator.updateQuestFinish(quest.getId(), status.getNpc(), intStore));
                break;
            case money:
                status = c.getQuest(quest);
                if (status.getForfeited() > 0) {
...
//		MapleQuest.getInstance(intStore).forceComplete(c, 0);
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        switch (type) {
            ...
            case nextQuest: {
                c.getClient().write(MaplePacketCreator.updateQuestFinish(quest.getId(), c.getQuest(quest).getNpc(), intStore));
                break;
            }
            case money: {
                c.gainMeso(intStore, true, true);
