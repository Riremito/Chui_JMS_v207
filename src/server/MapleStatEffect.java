                ...
                case 2300000:
                    ...
                    if (!mob.getStats().isBoss()) {
                        ...
                        if (absorbMp > 0) {
                            ...
                            applyto.getClient().write(MaplePacketCreator.showOwnBuffEffect(sourceid, 1, applyto.getLevel(), level));
...
//            applyfrom.getClient().write(MaplePacketCreator.enableActions());
            return false; //z
...
//                    applyto.getClient().write(MaplePacketCreator.enableActions());
                    return false;
                }
...
//                applyto.getClient().write(MaplePacketCreator.enableActions());
        ...
        if (expinc != 0) {
            ...
            applyto.getClient().write(MaplePacketCreator.showSpecialEffect(21));
            ...
            if (mobid > 0) {
                ...
                if (GameConstants.GMS) {
                    applyto.getClient().write(MaplePacketCreator.getCard(done ? sourceid : 0, 1));
     			 }
            }
            ...
            applyto.getClient().write(MaplePacketCreator.getPVPScore(x + bs, false));
            ...
            applyto.getClient().write(MaplePacketCreator.getPVPIceGage(x - iceGageCon));
	    applyto.applyIceGage(x - iceGageCon);
            ...
            applyto.getClient().write(MaplePacketCreator.rechargeCombo(applyto.getCombo()));
            ...
            if (portal != null) {
                applyto.getClient().write(MaplePacketCreator.dragonBlink(portal.getId()));
            ...
            if (type > 0 || effectedOnEnemy > 0) {
                for (MapleCharacter chr : applyto.getMap().getCharactersThreadsafe()) {
                    System.out.println("1 sourceid: "+ sourceid +" type: " + type + " chrid: " + chr.getId() + " applytoid " + applyto.getId());
            ...
            if (effectedOnEnemy > 0) {
                final int type = Integer.parseInt(applyto.getEventInstance().getProperty("type"));
                for (MapleCharacter chr : applyto.getMap().getCharactersThreadsafe()) {
                    System.out.println("2 sourceid: "+ sourceid +" type: " + type + " chrid: " + chr.getId() + " applytoid " + applyto.getId());
                ...
                if (sourceid == 2910000 || sourceid == 2910001) { //red flag
                    applyto.getClient().write(MaplePacketCreator.showOwnBuffEffect(sourceid, 13, applyto.getLevel(), level));
                    ...
                    applyto.getClient().write(MaplePacketCreator.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Effect", 0, 0));
                    ...
                    if (applyto.getTeam() == (sourceid - 2910000)) { //restore duh flag
                        ...
                        if (sourceid == 2910000) {
                            ...
                            applyto.getClient().write(MaplePacketCreator.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Tail/Red", 600000, 0));
                            ...
                            applyto.getClient().write(MaplePacketCreator.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Tail/Blue", 600000, 0));
                    ...
                    applyto.disease(mobSkill, mobSkillLevel);
                    System.out.println("3 sourceid: "+ sourceid +" type: " + type + " applytoid " + applyto.getId());
                    
/*                    final int type = Integer.parseInt(applyto.getEventInstance().getProperty("type"));
                    for (MapleCharacter chr : applyto.getMap().getCharactersThreadsafe()) {
                        System.out.println("3 sourceid: "+ sourceid +" type: " + type + " chrid: " + chr.getId() + " applytoid " + applyto.getId());
                        if (chr.getId() != applyto.getId() && (chr.getTeam() != applyto.getTeam() || type == 0)) {
                            chr.disease(mobSkill, mobSkillLevel);
                        }
                    }
        ...
        if (summonMovementType != null && (sourceid != 32111006 || (applyfrom.getBuffedValue(MapleBuffStat.REAPER) != null && !primary)) && !applyto.isClone()) {
            ...
            if (isBeholder()) {
                ...
                applyfrom.getClient().write(MaplePacketCreator.skillCooldown(sourceid, getCooldown()));
                ...
                applyfrom.getClient().write(MaplePacketCreator.enableActions()); //doubt we need this at all
            }
        } else if (isMechDoor()) {
            int newId = 0;
            ...
            applyto.getClient().write(MaplePacketCreator.mechPortal(door.getTruePosition()));
            if (!applyBuff) {
...
//                    applyto.getClient().write(MaplePacketCreator.enableActions());
        ...
        if (isSoulStone()) {
            if (applyfrom.getParty() != null) {
                ...
                for (MapleCharacter chr : awarded) {
                    ...
                    chr.getClient().write(MaplePacketCreator.showOwnBuffEffect(sourceid, 2, applyfrom.getLevel(), level));
            ...
            for (final MapleMapObject affectedmo : affecteds) {
                ...
                if (affected.getId() != applyfrom.getId() && (isGmBuff() || (applyfrom.inPVP() && affected.getTeam() == applyfrom.getTeam() && Integer.parseInt(applyfrom.getEventInstance().getProperty("type")) != 0) || (applyfrom.getParty() != null && affected.getParty() != null && applyfrom.getParty().getId() == affected.getParty().getId()))) {
                    if ((isResurrection() && !affected.isAlive()) || (!isResurrection() && affected.isAlive())) {
                        ...
                        affected.getClient().write(MaplePacketCreator.showOwnBuffEffect(sourceid, 2, applyfrom.getLevel(), level));
    ...
    public final void handleExtraPVP(MapleCharacter applyfrom, MapleCharacter chr) {
		...
		localstatups.put(MapleBuffStat.MORPH, x);
            ...
            chr.getClient().write(MaplePacketCreator.giveBuff(localsourceid, getDuration(), localstatups, this));
...
//	final ScheduledFuture<?> schedule = TimerManager.getInstance().schedule(cancelAction, ((starttime + 99999) - System.currentTimeMillis()));
    ...
    public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity) {
        final long starttime = System.currentTimeMillis();
        if (infinity) {
            applyto.getClient().write(MaplePacketCreator.giveEnergyChargeTest(0, duration / 1000));
            applyto.registerEffect(this, starttime, null, applyto.getId());
        } else {
            final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
    ...
    private final void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary, final int newDuration) {
        ...
        switch (sourceid) {
            ...
            case 5211007: {//dice
                ...
                applyto.getClient().write(MaplePacketCreator.showOwnDiceEffect(sourceid, zz, -1, level));
                if (zz <= 1) {
                    return;
                }
                localstatups = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                localstatups.put(MapleBuffStat.DICE_ROLL, zz);
                applyto.getClient().write(MaplePacketCreator.giveDice(zz, sourceid, localDuration, localstatups));
                normal = false;
                showEffect = false;
                break;
            }
            case 5320007: {//dice
                ...
                applyto.getClient().write(MaplePacketCreator.showOwnDiceEffect(sourceid, zz, zz2 > 0 ? -1 : 0, level));
                if (zz <= 1 && zz2 <= 1) {
                    return;
                }
            ...
            case 33101006: {//jaguar oshi
                ...
                applyto.getClient().write(MaplePacketCreator.giveBuff(sourceid, localDuration, localstatups, this));
                normal = false;
                break;
            }
            case 8006:
            ...
            case 5220011: {// Bullseye
                if (applyto.getFirstLinkMid() > 0) {
                    ...
                    applyto.getClient().write(MaplePacketCreator.giveHoming(sourceid, applyto.getFirstLinkMid(), 1));
                } else {
                    return;
                }
                normal = false;
            ...
            case 2320011: //arcane aim
                if (applyto.getFirstLinkMid() > 0) {
                    applyto.getClient().write(MaplePacketCreator.giveArcane(applyto.getAllLinkMid(), localDuration));
                } else {
                    return;
                }
                normal = false;
            ...
            case 32111005: { //body boost
                ...
                applyto.getClient().write(MaplePacketCreator.giveBuff(sourcez, localDuration, stat, this));
                normal = false;
                break;
            }
            case 32001003: {//dark aura
            ...
            case 32110007: {//fallthrough intended
                ...
                applyto.getClient().write(MaplePacketCreator.giveBuff(sourceid, localDuration, statt, this));
                statt.clear();
            ...
            case 32120000: {
                ...
                applyto.getClient().write(MaplePacketCreator.giveBuff(32001003, localDuration, statt, this));
                statt.clear();
                statt.put(MapleBuffStat.DARK_AURA, x);
                applyto.getClient().write(MaplePacketCreator.giveBuff(sourceid, localDuration, statt, this));
                applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), statt, this), false);
            ...
            case 32110000: {
                ...
                applyto.getClient().write(MaplePacketCreator.giveBuff(sourceid == 32110000 ? 32101002 : sourceid, localDuration, statt, this));
                statt.clear();
            ...
            case 32120001: {
                ...
                applyto.getClient().write(MaplePacketCreator.giveBuff(sourceid == 32120001 ? 32101003 : sourceid, localDuration, statt, this));
                statt.clear();
            ...
            default:
                if (isPirateMorph()) {
					...
					if (isIceKnight()) {
						...
						applyto.getClient().write(MaplePacketCreator.giveBuff(0, localDuration, stat, this));
					}
                    final EnumMap<MapleBuffStat, Integer> stat = new EnumMap<MapleBuffStat, Integer>(MapleBuffStat.class);
                    stat.put(MapleBuffStat.MORPH, getMorph(applyto));
                    applyto.getMap().broadcastMessage(applyto, MaplePacketCreator.giveForeignBuff(applyto.getId(), stat, this), false);
                    ...
                    if (mountid != 0 && mountid2 != 0) {
                        ...
                        applyto.getClient().write(MaplePacketCreator.giveMount(mountid2, sourceid, stat));
        ...
        if (isMechPassive()) {
            applyto.getClient().write(MaplePacketCreator.showOwnBuffEffect(sourceid - 1000, 1, applyto.getLevel(), level, (byte) 1));
        }
        ...
        if (normal && localstatups.size() > 0) {
            applyto.getClient().write(MaplePacketCreator.giveBuff((skill ? sourceid : -sourceid), localDuration, maskedStatups == null ? localstatups : maskedStatups, this));
        }
        final long starttime = System.currentTimeMillis();
