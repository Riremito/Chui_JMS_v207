...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        chr.get().getClient().write(MaplePacketCreator.TradeMessage(tradingslot, (byte) 0x07));
    }
    ...
    public final void cancel(final MapleClient c, final MapleCharacter chr, final int unsuccessful) {
        ...
        c.write(MaplePacketCreator.getTradeCancel(tradingslot, unsuccessful));
    }
    ...
    public final void setMeso(final int meso) {
        ...
        if (chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            chr.get().getClient().write(MaplePacketCreator.getTradeMesoSet((byte) 0, this.meso));
            if (partner != null) {
    ...
    public final void addItem(final Item item) {
        ...
        items.add(item);
        chr.get().getClient().write(MaplePacketCreator.getTradeItemAdd((byte) 0, item));
        if (partner != null) {
    ...
    public final void chat(final String message) {
        if (!CommandProcessor.processCommand(chr.get().getClient(), message, CommandType.TRADE)) {
            ...
            if (partner != null) {
                partner.getChr().getClient().write(PlayerShopPacket.shopChat(chr.get().getName() + " : " + message, 1));
            }
        }
    ...
    public final void chatAuto(final String message) {
        ...
        if (partner != null) {
            partner.getChr().getClient().write(PlayerShopPacket.shopChat(message, 1));
        }
        if (chr.get().getClient().isMonitored()) { //Broadcast info even if it was a command.
