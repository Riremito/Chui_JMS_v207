...
public class MapleAchievement {
    ...
    public void finishAchievement(MapleCharacter chr) {
        ...
        if (notice && !chr.isGM()) {
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[成就] Congratulations to " + chr.getName() + " on " + name + " and rewarded with " + (GameConstants.GMS ? (reward/2) : reward) + " cash!"));
        } else {
            chr.getClient().write(MaplePacketCreator.serverNotice(5, "[成就] You've gained " + (GameConstants.GMS ? (reward/2) : reward) + " Cash as you " + name + "."));
        }
    }
}
