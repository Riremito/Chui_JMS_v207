...
public class ChangeableStats extends OverrideMonsterStats {
    ...
    public ChangeableStats(MapleMonsterStats stats, int newLevel, boolean pqMob) {
        ...
        final double hpRatio = (double) stats.getDefaultHp() / (double) stats.getDefaultExp();
        ...
        exp = (int) Math.round((!stats.isBoss() ? (GameConstants.getMonsterHP(newLevel) / hpRatio) : (stats.getDefaultExp() * mod)) * mod * pqMod);
