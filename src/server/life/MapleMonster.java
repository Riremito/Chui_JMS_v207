...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    private final void initWithStats(final MapleMonsterStats stats) {
        ...
        this.stats = stats;
        if (!stats.isBoss()) {
            maxhp = stats.getDefaultHp() * Randomizer.rand(70, 190) / 100;
        } else {
    ...
    public final long getMobMaxHp() {
        ...
        return maxhp/*stats.getDefaultHp()*/;
    }
    ...
    public final int getMobExp() {
        ...
        int exp;
        if (!stats.isBoss()) {
            exp = stats.getDefaultExp() * Randomizer.rand(70, 120) / 100;
        } else {
                        ...
                        for (final AttackingMapleCharacter cattacker : mattacker.getAttackers()) {
                            if (cattacker.getAttacker().getMap() == from.getMap()) { // current attacker is on the map of the monster
                                if (cattacker.getLastAttackTime() >= System.currentTimeMillis() - 4000) {
                                    cattacker.getAttacker().getClient().write(MobPacket.showMonsterHP(getObjectId(), getHPPercent()));
                                }
                            }
                        }
                    }*/
...
/*                        case 3:
    ...
    public final void sendSpawnData(final MapleClient client) {
        ...
        client.write(MobPacket.spawnMonster(this, fake && linkCID <= 0 ? -4 : -1, 0));
        sendStatus(client);
    ...
    public void setNodePacket(final OutPacket np) {
        this.nodepack = np;
    }
