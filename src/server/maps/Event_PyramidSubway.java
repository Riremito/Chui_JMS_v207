...
public class Event_PyramidSubway {
	...
	bar = 0;
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    chr.getClient().write(MaplePacketCreator.getClock(time));
                    chr.getClient().write(MaplePacketCreator.showEffect("killing/first/number/" + stage));
                    chr.getClient().write(MaplePacketCreator.showEffect("killing/first/stage"));
                    chr.getClient().write(MaplePacketCreator.showEffect("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().write(MaplePacketCreator.getClock(time));
            c.getClient().write(MaplePacketCreator.showEffect("killing/first/number/" + stage));
            c.getClient().write(MaplePacketCreator.showEffect("killing/first/stage"));
            c.getClient().write(MaplePacketCreator.showEffect("killing/first/start"));
            fullUpdate(c, stage);
        }
	...
	c.getTrait(MapleTraitType.will).addExp((type + 2) * 8, c);
        c.getClient().write(MaplePacketCreator.showEffect("killing/clear"));
        c.getClient().write(MaplePacketCreator.sendPyramidResult(rank, exp));
        dispose(c);
    }
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().write(MaplePacketCreator.sendPyramidUpdate(bar));
    }

    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().write(MaplePacketCreator.showEffect(effect));
    }

    public final void broadcastEnergy(final MapleCharacter c, final String type, final int amount) {
        c.getClient().write(MaplePacketCreator.sendPyramidEnergy(type, String.valueOf(amount)));
    }
    ...
    private static final void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().write(MaplePacketCreator.showEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().write(MaplePacketCreator.showEffect("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().write(MaplePacketCreator.showEffect("killing/fail"));
        }
