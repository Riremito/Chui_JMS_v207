    ...
    public void dropItems(boolean meso, int mesoChance, int minMeso, int maxMeso, int minItems) {
        ...
        for (final ReactorDropEntry d : items) {
            if (d.itemId == 0) {
                ...
                if (reactor.getMap().spawnReactorMesoDrop(mesoDrop, dropPos, reactor, getPlayer(), false, (byte) 0, delay)) {
                    dropPos.x += 20;
                }
            } else {
                Item drop;
                ...
                if (reactor.getMap().spawnReactorItemDrop(reactor, getPlayer(), drop, dropPos, false, false, delay)) {
                    dropPos.x += 20;
                }
            }
...
//        getPlayer().setFatigue((byte) (getPlayer().getFatigue() + 1));
    ...
    public void doHarvest() { //TODO LEGEND
        ...
        final String pName = (getReactor().getReactorId() < 200000 ? "薬草採集" : "採鉱");
        ...
        cancelHarvest(succ);
        playerMessage(-5, pName + "の熟練度が増加した。(+" + masteryIncrease + ")");
        if (getPlayer().addProfessionExp(pID, masteryIncrease)) {
            playerMessage(-5, pName + "のレベルが上がりました。");
        }
    }
}
