...
public abstract class AbstractPlayerInteraction {
    ...
    public final void warp(final int map, final int portal) {
        ...
        if (portal != 0 && map == c.getPlayer().getMapId()) { //test
            ...
            if (portalPos.distanceSq(getPlayer().getTruePosition()) < 90000.0) { //estimation
                c.write(MaplePacketCreator.instantMapWarp((byte) portal)); //until we get packet for far movement, this will do
    ...
    public final void warp(final int map, String portal) {
        ...
        if (map == c.getPlayer().getMapId()) { //test
            ...
            if (portalPos.distanceSq(getPlayer().getTruePosition()) < 90000.0) { //estimation
                c.getPlayer().checkFollow();
                c.write(MaplePacketCreator.instantMapWarp((byte) c.getPlayer().getMap().getPortal(portal).getId()));
    ...
    public final void playPortalSE() {
        c.write(MaplePacketCreator.showOwnBuffEffect(0, 7, 1, 1));
    }
    ...
    public final void showQuestMsg(final String msg) {
        c.write(MaplePacketCreator.showQuestMsg(msg));
    }
        ...
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id) && !GameConstants.isBullet(id)) {
                ...
                if (id / 10000 == 114 && name != null && name.length() > 0) { //medal
                    final String msg = "勲章 - " + name + "を獲得しました！";
//                    cg.getPlayer().dropMessage(-1, msg);
        ...
        cg.write(MaplePacketCreator.getShowItemGain(id, quantity, true));
    }
    ...
    public final boolean removeItem(final int id) { //quantity 1
        if (MapleInventoryManipulator.removeById_Lock(c, GameConstants.getInventoryType(id), id)) {
            c.write(MaplePacketCreator.getShowItemGain(id, (short) -1, true));
            return true;
        }
        return false;
    }
    ...
    public final void gainCloseness(final int closeness, final int index) {
        ...
        if (pet != null) {
            ...
            getClient().write(PetPacket.updatePet(pet, getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
        }
    }
    ...
    public final void dojo_getUp() {
        c.write(MaplePacketCreator.updateInfoQuest(1207, "pt=1;min=4;belt=1;tuto=1")); //todo
    ...
    public final void evanTutorial(final String data, final int v1) {
        c.write(MaplePacketCreator.getEvanTutorial(data));
    }
    ...
    public final void EnableUI(final short i) {
        c.write(UIPacket.IntroEnableUI(i));
    }
    ...
    public void showMapEffect(String path) {
        getClient().write(UIPacket.MapEff(path));
    }
...
//	FileoutputUtil.log(FileoutputUtil.PQ_Log, text);
    ...
    public void trembleEffect(int type, int delay) {
        c.write(MaplePacketCreator.trembleEffect(type, delay));
    }
    ...
    public void sendDirectionStatus(int key, int value) {
        c.write(UIPacket.getDirectionInfo(key, value));
        c.write(UIPacket.getDirectionStatus(true));
    }

    public void sendDirectionInfo(String data) {
        c.write(UIPacket.getDirectionInfo(data, 2000, 0, -100, 0));
