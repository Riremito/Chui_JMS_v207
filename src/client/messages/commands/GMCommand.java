...
public class GMCommand {
    ...
    public static class SP extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            c.write(MaplePacketCreator.updateSp(c.getPlayer(), false));
            return 1;
        }
    }
...
/*                if (!c.getPlayer().isSuperGM()) {
    ...
    public static class LockItem extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (Item item : chr.getInventory(type).listById(itemid)) {
                ...
                chr.getClient().write(MaplePacketCreator.updateSpecialItemUse(item, type.getType(), item.getPosition(), true, chr));
            }
            if (type == MapleInventoryType.EQUIP) {
                type = MapleInventoryType.EQUIPPED;
                for (Item item : chr.getInventory(type).listById(itemid)) {
                    item.setFlag((byte) (item.getFlag() | ItemFlag.LOCK.getValue()));
                    //chr.getClient().write(MaplePacketCreator.updateSpecialItemUse(item, type.getType()));
                }
            }
    ...
    public static class Notice extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            OutPacket packet = MaplePacketCreator.serverNotice(type, sb.toString());
            if (range == 0) {
    ...
    public static class Yellow extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            OutPacket packet = MaplePacketCreator.yellowChat((splitted[0].equals("!y") ? ("[" + c.getPlayer().getName() + "] ") : "") + StringUtil.joinStringFrom(splitted, 2));
            if (range == 0) {
                c.getPlayer().getMap().broadcastMessage(packet);
            } else if (range == 1) {
                ChannelServer.getInstance(c.getChannel()).broadcastPacket(packet);
    ...
    public static class WhatsMyIP extends CommandExecute {
        ...
        @Override
        public int execute(MapleClient c, String[] splitted) {
            c.getPlayer().dropMessage(5, "IP: " + c.getRemoteAddress().toString().split(":")[0]);
            return 1;
        }
    }
