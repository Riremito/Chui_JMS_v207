...
public class MonsterFamiliar extends AnimatedMapleMapObject implements Serializable {
    ...
    public void addFatigue(MapleCharacter owner, int f) {
        ...
        owner.getClient().write(MaplePacketCreator.updateFamiliar(this));
    ...
    public void sendSpawnData(MapleClient client) {
        client.write(MaplePacketCreator.spawnFamiliar(this, true));
    }

    @Override
    public void sendDestroyData(MapleClient client) {
        client.write(MaplePacketCreator.spawnFamiliar(this, false));
    }
    ...
    public void writeRegisterPacket(OutPacket outPacket, boolean chr) {
        outPacket.encodeInt(getCharacterId()); //lol
        outPacket.encodeInt(getFamiliar());
        outPacket.encodeZeroBytes(13);
        outPacket.encodeByte(chr ? 1 : 0);
        outPacket.encodeShort(getVitality());
        outPacket.encodeInt(getFatigue());
        outPacket.encodeLong(PacketHelper.getTime(getVitality() >= 3 ? System.currentTimeMillis() : -2));
        outPacket.encodeLong(PacketHelper.getTime(System.currentTimeMillis()));
        outPacket.encodeLong(PacketHelper.getTime(getExpiry()));
        outPacket.encodeShort(getVitality());
    }
}
