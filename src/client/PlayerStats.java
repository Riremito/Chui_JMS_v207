...
public class PlayerStats implements Serializable {
    ...
    public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
        ...
        boolean pvp = (chra.getEventInstance() != null && chra.getEventInstance().getName().startsWith("PVP"));
        if (pvp) {
            eff = chra.getStatForBuff(MapleBuffStat.MORPH);
            boolean iceknight = eff != null ? eff.getSourceId() % 10000 == 1105 : false;

            int h = GameConstants.getMaxHP_PVP(chra.getJob(), Integer.parseInt(chra.getEventInstance().getProperty("lvl")), iceknight);
            localmaxhp = h;
...
/*            eff = chra.getStatForBuff(MapleBuffStat.MORPH);
        ...
        if (pvp) {
            System.out.println("hp" + hp + " localmaxhp " + localmaxhp + " maxhp" + maxhp);
        }
    ...
    public boolean checkEquipLevels(final MapleCharacter chr, int gain) {
                    ...
                    if (GameConstants.getStatFromWeapon(eq.getItemId()) == null && GameConstants.getMaxLevel(eq.getItemId()) < (lvlz + i) && Math.random() < 0.1 && eq.getIncSkill() <= 0 && ii.getEquipSkills(eq.getItemId()) != null) {
                        for (int zzz : ii.getEquipSkills(eq.getItemId())) {
                            ...
                            if (skil != null && skil.canBeLearnedBy(chr.getJob())) { //dont go over masterlevel :D
                                ...
                                chr.dropMessage(5, "アイテム効果 スキル: " + skil.getName() + " + 1");
                            }
                        }
                    }
                }
        ...
        if (changed) {
            chr.equipChanged();
            chr.getClient().write(MaplePacketCreator.showItemLevelupEffect());
        ...
        for (Equip eqq : all) {
            if (eqq != null && eqq.getDurability() == 0 && eqq.getPosition() < 0) { //> 0 went to negative
                if (chr.getInventory(MapleInventoryType.EQUIP).isFull()) {
                    chr.getClient().write(MaplePacketCreator.getInventoryFull());
                    chr.getClient().write(MaplePacketCreator.getShowInventoryFull());
                    return false;
                }
    ...
    public final void connectData(final OutPacket outPacket) {
        outPacket.encodeShort(str); // str
        outPacket.encodeShort(dex); // dex
        outPacket.encodeShort(int_); // int
        outPacket.encodeShort(luk); // luk

        outPacket.encodeInt(hp); // hp -- INT after bigbang
        outPacket.encodeInt(maxhp); // maxhp
        outPacket.encodeInt(mp); // mp
        outPacket.encodeInt(maxmp); // maxmp
    }
