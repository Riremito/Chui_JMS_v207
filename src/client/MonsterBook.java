...
public class MonsterBook implements Serializable {
    ...
    public void writeCharInfoPacket(OutPacket outPacket) {
        ...
        outPacket.encodeInt(setScore);
        outPacket.encodeInt(currentSet);
        outPacket.encodeInt(finishedSets);
    }

    public void writeFinished(OutPacket outPacket) {
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        outPacket.encodeByte(1);
        outPacket.encodeShort(cardItems.size());
        ...
        outPacket.encodeShort(fullCards); //which cards of all you have; more efficient than writing each card
        ...
        for (int i = 0; i < fullCards; i++) {
            ...
            outPacket.encodeByte(maskToWrite);
        }

        final int fullSize = (cardItems.size() / 2) + (cardItems.size() % 2 > 0 ? 1 : 0);
        outPacket.encodeShort(fullSize); //i honestly don't know the point of this, is it to signify yes/no if you have the card or not?... which is already done...?
        for (int i = 0; i < fullSize; i++) {
            outPacket.encodeByte(i == (cardItems.size() / 2) ? 1 : 0x11);
        }
    }

    public void writeUnfinished(OutPacket outPacket) {
        outPacket.encodeByte(0);
        outPacket.encodeShort(cardItems.size());
        for (int i : cardItems) {
            outPacket.encodeShort(i % 10000);
            outPacket.encodeByte(1); //whether you have the card or not? idk
        }
    }
...
//            c.getPlayer().dropMessage(-6, "Book entry updated - " + cardname);
            c.write(MaplePacketCreator.showSpecialEffect(16));
            cards.put(cardid, 2);
            if (GameConstants.GMS) {
                ...
                if (rr > 0) {
                    ...
                    c.write(MaplePacketCreator.showSpecialEffect(42));
                    if (rr > 1) {
