...
public class StatsHandling {
    ...
    public static final void DistributeAP(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
	...
	for (int i : GameConstants.blockedSkills) {
        ...
        if ((remainingSp > 0 && curLevel + 1 <= maxlevel) && skill.canBeLearnedBy(chr.getJob())) {
            ...
            c.write(MaplePacketCreator.updateSp(chr, false));
    ...
    public static final void AutoAssignAP(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        chr.updateTick(inPacket.decodeInt());
        inPacket.skip(4);
        if (inPacket.getUnreadAmount() < 16) {
            return;
        }
        final int PrimaryStat = (int)inPacket.decodeLong();
        final int amount = inPacket.decodeInt();
        final int SecondaryStat = (int)inPacket.decodeLong();
        final int amount2 = inPacket.decodeInt();
