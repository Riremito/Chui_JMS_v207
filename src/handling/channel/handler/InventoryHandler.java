...
public class InventoryHandler {
    ...
    public static final void ItemMove(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().hasBlockedInventory()) { //hack
            return;
        }
        c.getPlayer().setScrolledPosition((short) 0);
        c.getPlayer().updateTick(inPacket.decodeInt());
        final MapleInventoryType type = MapleInventoryType.getByType(inPacket.decodeByte()); //04
        final short src = inPacket.decodeShort();                                            //01 00
        final short dst = inPacket.decodeShort();                                            //00 00
        final short quantity = inPacket.decodeShort();                                       //53 01
    ...
    public static final void SwitchBag(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().hasBlockedInventory()) { //hack
            return;
        }
        c.getPlayer().setScrolledPosition((short) 0);
        c.getPlayer().updateTick(inPacket.decodeInt());
        final short src = (short) inPacket.decodeInt();                                       //01 00
        final short dst = (short) inPacket.decodeInt();                                            //00 00
        if (src < 100 || dst < 100) {
            return;
        }
    ...
    public static final void MoveBag(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().hasBlockedInventory()) { //hack
            return;
        }
        c.getPlayer().setScrolledPosition((short) 0);
        c.getPlayer().updateTick(inPacket.decodeInt());
        final boolean srcFirst = inPacket.decodeInt() > 0;
        short dst = (short) inPacket.decodeInt();                                       //01 00
        if (inPacket.decodeByte() != 4) { //must be etc) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        short src = inPacket.decodeShort();                                            //00 00
        MapleInventoryManipulator.move(c, MapleInventoryType.ETC, srcFirst ? dst : src, srcFirst ? src : dst);
    }

    public static final void ItemSort(final InPacket inPacket, final MapleClient c) {
        c.getPlayer().updateTick(inPacket.decodeInt());
        c.getPlayer().setScrolledPosition((short) 0);
        final MapleInventoryType pInvType = MapleInventoryType.getByType(inPacket.decodeByte());
        ...
        c.write(MaplePacketCreator.finishedSort(pInvType.getType()));
        c.write(MaplePacketCreator.enableActions());
    }

    public static final void ItemGather(final InPacket inPacket, final MapleClient c) {
        ...
        c.write(MaplePacketCreator.finishedGather(mode));
        c.write(MaplePacketCreator.enableActions());
        itemMap.clear();
        sortedItems.clear();
    }
    ...
    public static final void UseItem(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void UseCosmetic(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void UseReturnScroll(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void UseMagnify(final InPacket inPacket, final MapleClient c) {
        c.getPlayer().updateTick(inPacket.decodeInt());
        c.getPlayer().setScrolledPosition((short) 0);
        final byte src = (byte) inPacket.decodeShort();
        ...
        final Item toReveal = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((byte) inPacket.decodeShort());
        ...
        if (eqq.getState() == 1 && (insight || magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12) || (magnify.getItemId() == 2460001 && reqLevel <= 7) || (magnify.getItemId() == 2460000 && reqLevel <= 3))) {
            ...
            if (!insight) {
                c.write(MaplePacketCreator.scrolledItem(magnify, toReveal, false, true));
    ...
    public static final boolean UseUpgradeScroll(final short slot, final short dst, final short ws, final MapleClient c, final MapleCharacter chr, final int vegas) {
        ...
        if (whiteScroll) {
            ...
            if (vegas == 0) {
                c.write(MaplePacketCreator.pamSongUI());
            }
        }

        if (scrollSuccess == Equip.ScrollResult.CURSE) {
            c.write(MaplePacketCreator.scrolledItem(scroll, toScroll, true, false));
            if (dst < 0) {
            ...
            c.write(MaplePacketCreator.scrolledItem(scroll, scrolled, false, false));
        }
    ...
    public static final void UseCatchItem(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        c.getPlayer().updateTick(inPacket.decodeInt());
        c.getPlayer().setScrolledPosition((short) 0);
        final byte slot = (byte) inPacket.decodeShort();
        final int itemid = inPacket.decodeInt();
        final MapleMonster mob = chr.getMap().getMonsterByOid(inPacket.decodeInt());
        final Item toUse = chr.getInventory(MapleInventoryType.USE).getItem(slot);
        ...
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemid && mob != null && !chr.hasBlockedInventory() && itemid / 10000 == 227 && MapleItemInformationProvider.getInstance().getCardMobId(itemid) == mob.getId()) {
            if (!MapleItemInformationProvider.getInstance().isMobHP(itemid) || mob.getHp() <= mob.getMobMaxHp() / 2) {
                ...
                c.write(MaplePacketCreator.catchMob(mob.getId(), itemid, (byte) 0));
            }
        }
        c.write(MaplePacketCreator.enableActions());
    }

    public static final void UseMountFood(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        c.getPlayer().updateTick(inPacket.decodeInt());
        final byte slot = (byte) inPacket.decodeShort();
        final int itemid = inPacket.decodeInt(); //2260000 usually
        final Item toUse = chr.getInventory(MapleInventoryType.USE).getItem(slot);
    ...
    public static final void UseScriptedNPCItem(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        c.getPlayer().updateTick(inPacket.decodeInt());
        final byte slot = (byte) inPacket.decodeShort();
        final int itemId = inPacket.decodeInt();
        final Item toUse = chr.getInventory(GameConstants.getInventoryType(itemId)).getItem(slot);
    ...
    public static final void UseSummonBag(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void UseTreasureChest(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void UseCashItem(final InPacket inPacket, final MapleClient c) {
        ...
        switch (itemId) {
            ...
            case 5043000: { // NPC Teleport Rock
                final short questid = inPacket.decodeShort();
            ...
            case 5040001: { // Teleport Coke
                used = UseTeleRock(inPacket, c, itemId);
                break;
            }
            case 5450005: {
            ...
            case 5050000: { // AP Reset
                ...
                final int apto = GameConstants.GMS ? (int) inPacket.decodeLong() : inPacket.decodeInt();
                final int apfrom = GameConstants.GMS ? (int) inPacket.decodeLong() : inPacket.decodeInt();
            ...
            case 5220084: {//booster pack
                    ...
                    for (int i = 0; i < familiars.length; i++) {
                        ...
                        for (Map.Entry<Integer, StructFamiliar> f : MapleItemInformationProvider.getInstance().getFamiliars().entrySet()) {
                            if (Randomizer.nextInt(500) == 0 && ((i < 2 && f.getValue().grade == 0 || (i == 2 && f.getValue().grade != 0)))) {
                                ...
                                c.write(MTSCSPacket.getBoosterFamiliar(c.getPlayer().getId(), f.getKey(), 0));
                ...
                c.write(MTSCSPacket.getBoosterPack(familiars[0], familiars[1], familiars[2]));
                c.write(MTSCSPacket.getBoosterPackClick());
                c.write(MTSCSPacket.getBoosterPackReveal());
                break;
            }
            ...
            case 5050009: {
                ...
                int skill1 = inPacket.decodeInt();
                int skill2 = inPacket.decodeInt();
            ...
            case 5500000: { // Magic Hourglass 1 day
                final Item item = c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem(inPacket.decodeShort());
                final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                final int days = 1;
            ...
            case 5534000: { //tims lab
                final Item item = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((byte) inPacket.decodeInt());
                if (item != null) {
                    final Equip eq = (Equip) item;
                    if (eq.getState() == 0) {
                        eq.resetPotential();
                        c.write(MaplePacketCreator.scrolledItem(toUse, item, false, true));
            ...
            case 5521000: { // Karma
                final MapleInventoryType type = MapleInventoryType.getByType((byte) inPacket.decodeInt());
                final Item item = c.getPlayer().getInventory(type).getItem((byte) inPacket.decodeInt());
                ...
                if (item != null && !ItemFlag.KARMA_ACC.check(item.getFlag()) && !ItemFlag.KARMA_ACC_USE.check(item.getFlag())) {
                    if (MapleItemInformationProvider.getInstance().isShareTagEnabled(item.getItemId())) {
                        ...
                        c.write(MaplePacketCreator.updateSpecialItemUse(item, type.getType(), item.getPosition(), true, c.getPlayer()));
                        used = true;
                    }
                }
                break;
            ...
            case 5570000: { // Vicious Hammer
                inPacket.decodeInt(); // Inventory type, Hammered eq is always EQ.
                final Equip item = (Equip) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((byte) inPacket.decodeInt());
                ...
                if (item != null) {
                    if (GameConstants.canHammer(item.getItemId()) && MapleItemInformationProvider.getInstance().getSlots(item.getItemId()) > 0 && item.getViciousHammer() < 2) {
                        ...
                        if (GameConstants.GMS) {
                            c.write(MTSCSPacket.ViciousHammer(true, (byte) 0));
                            c.write(MTSCSPacket.ViciousHammer(false, (byte) 0));
                        } else {
                            cc = true;
                        }
            ...
            case 5610000: { // Vega 30
                inPacket.decodeInt(); // Inventory type, always eq
                final short dst = (short) inPacket.decodeInt();
                inPacket.decodeInt(); // Inventory type, always use
                final short src = (short) inPacket.decodeInt();
            ...
            case 5077000: { // 3 line Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    final byte numLines = inPacket.decodeByte();
                    if (numLines > 3) {
                        return;
                    }
                    ...
                    for (int i = 0; i < numLines; i++) {
                        message = inPacket.decodeString();
                        if (message.length() > 65) {
                            break;
                        }
                        messages.add(c.getPlayer().getName() + " : " + message);
                    }
                    final boolean ear = inPacket.decodeByte() > 0;
            ...
            case 5073000: { // Heart Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    final boolean ear = inPacket.decodeByte() != 0;
                    World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(14/*9*/, c.getChannel(), sb.toString(), ear));
                    used = true;
                } else {
                    c.getPlayer().dropMessage(5, "The usage of Megaphone is currently disabled.");
                }
            ...
            case 5074000: { // Skull Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    World.Broadcast.broadcastSmega(MaplePacketCreator.serverNotice(15/*10*/, c.getChannel(), sb.toString(), ear));
                    used = true;
                } else {
                    c.getPlayer().dropMessage(5, "The usage of Megaphone is currently disabled.");
                }
            ...
            case 5076000: { // Item Megaphone
                ...
                if (!c.getChannelServer().getMegaphoneMuteState()) {
                    ...
                    Item item = null;
                    if (inPacket.decodeByte() == 1) { //item
                        byte invType = (byte) inPacket.decodeInt();
                        byte pos = (byte) inPacket.decodeInt();
            ...
            case 5075005: {
                ...
                if (tvType == 3) {
                    inPacket.decodeByte(); //who knows
                }
                boolean ear = tvType != 1 && tvType != 2 && inPacket.decodeByte() > 1; //for tvType 1/2, there is no byte. 
                MapleCharacter victim = tvType == 1 || tvType == 4 ? null : c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.decodeString()); //for tvType 4, there is no string.
            ...
            case 5090000: { // Note
                final String sendTo = inPacket.decodeString();
            ...
            case 5190000: { // Pet Flags
                final int uniqueid = (int) inPacket.decodeLong();
                ...
                if (zz != null && !zz.check(pet.getFlags())) {
                    ...
                    c.write(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
                    c.write(MaplePacketCreator.enableActions());
                    c.write(MTSCSPacket.changePetFlag(uniqueid, true, zz.getValue()));
                    used = true;
                }
                break;
            }
            ...
            case 5191000: { // Pet Flags
                ...
                if (zz != null && zz.check(pet.getFlags())) {
                    ...
                    c.write(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
                    c.write(MaplePacketCreator.enableActions());
                    c.write(MTSCSPacket.changePetFlag(uniqueid, false, zz.getValue()));
                    used = true;
                }
                break;
            }
            ...
            case 5501002: { //expiry mount
                final Skill skil = SkillFactory.getSkill(inPacket.decodeInt());
            ...
            case 5170000: { // Pet name change
                ...
                String nName = inPacket.decodeString();
                for (String z : GameConstants.RESERVED) {
            ...
            case 5240028: { // Pet food
                ...
                c.write(PetPacket.updatePet(pet, c.getPlayer().getInventory(MapleInventoryType.CASH).getItem(pet.getInventoryPosition()), true));
            ...
            case 5230000: {// owl of minerva
                final int itemSearch = inPacket.decodeInt();
                final List<HiredMerchant> hms = c.getChannelServer().searchMerchant(itemSearch);
                if (hms.size() > 0) {
                    c.write(MaplePacketCreator.getOwlSearched(itemSearch, hms));
                    used = true;
                } else {
            ...
            case 5370000: { // Chalkboard
                ...
                c.getPlayer().setChalkboard(inPacket.decodeString());
                break;
            }
            case 5079000:
            case 5079001:
            ...
            default:
                if (itemId / 10000 == 512) {
                    ...
                    final String ourMsg = inPacket.decodeString();
                    if (!msg.contains("%s")) {
                        msg = ourMsg;
                    } else {
                    ...
                    if (mesars > 0 && c.getPlayer().getMeso() < (Integer.MAX_VALUE - mesars)) {
                        ...
                        if (Math.random() > 0.1) {
                            ...
                            c.write(MTSCSPacket.sendMesobagSuccess(gainmes));
                        } else {
                            c.write(MTSCSPacket.sendMesobagFailed());
                        }
                    }
...
//                        c.getPlayer().dropMessage(6, "使用日拋隱形眼鏡出現錯誤。");
    ...
    public static final void Pickup_Player(final InPacket inPacket, MapleClient c, final MapleCharacter chr) {
        if (c.getPlayer().hasBlockedInventory()) { //hack
            return;
        }
        chr.updateTick(inPacket.decodeInt());
        c.getPlayer().setScrolledPosition((short) 0);
        inPacket.skip(1); // or is this before tick?
        final Point Client_Reportedpos = inPacket.decodePosition();
        if (chr == null || chr.getMap() == null) {
            return;
        }
        final MapleMapObject ob = chr.getMap().getMapObject(inPacket.decodeInt(), MapleMapObjectType.ITEM);
    ...
    public static final void Pickup_Pet(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
        ...
        c.getPlayer().setScrolledPosition((short) 0);
        final byte petz = (byte) (GameConstants.GMS ? (c.getPlayer().getPetIndex((int) inPacket.decodeLong())) : inPacket.decodeInt());
        final MaplePet pet = chr.getPet(petz);
        inPacket.skip(1); // [4] Zero, [4] Seems to be tickcount, [1] Always zero
        chr.updateTick(inPacket.decodeInt());
        final Point Client_Reportedpos = inPacket.decodePosition();
        final MapleMapObject ob = chr.getMap().getMapObject(inPacket.decodeInt(), MapleMapObjectType.ITEM);
    ...
    private static final boolean getIncubatedItems(MapleClient c, int itemId) {
        ...
        c.write(MaplePacketCreator.getPeanutResult(id1, (short) 1, id2, (short) 1, itemId));
    ...
    public static final void OwlMinerva(final InPacket inPacket, final MapleClient c) {
        final byte slot = (byte) inPacket.decodeShort();
        final int itemid = inPacket.decodeInt();
    ...
    public static final void Owl(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().haveItem(5230000, 1, true, false) || c.getPlayer().haveItem(2310000, 1, true, false)) {
            if (c.getPlayer().getMapId() >= 910000000 && c.getPlayer().getMapId() <= 910000022) {
                c.write(MaplePacketCreator.getOwlOpen());
            } else {
    ...
    public static final void OwlWarp(final InPacket inPacket, final MapleClient c) {
        c.write(MaplePacketCreator.enableActions());
        if (c.getPlayer().getMapId() >= 910000000 && c.getPlayer().getMapId() <= 910000022 && !c.getPlayer().hasBlockedInventory()) {
            ...
            final int map = inPacket.decodeInt();
    ...
    public static final void PamSong(InPacket inPacket, MapleClient c) {
        final Item pam = c.getPlayer().getInventory(MapleInventoryType.CASH).findById(5640000);
        if (inPacket.decodeByte() > 0 && c.getPlayer().getScrolledPosition() != 0 && pam != null && pam.getQuantity() > 0) {
    ...
    public static final void TeleRock(InPacket inPacket, MapleClient c) {
        final byte slot = (byte) inPacket.decodeShort();
        final int itemId = inPacket.decodeInt();
        final Item toUse = c.getPlayer().getInventory(MapleInventoryType.USE).getItem(slot);
    ...
    public static final boolean UseTeleRock(InPacket inPacket, MapleClient c, int itemId) {
        boolean used = false;
        if (itemId == 5041001 || itemId == 5040004) {
            inPacket.decodeByte(); //useless
        }
        if (inPacket.decodeByte() == 0) { // Rocktype
            final MapleMap target = c.getChannelServer().getMapFactory().getMap(inPacket.decodeInt());
            ...
            final MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.decodeString());
