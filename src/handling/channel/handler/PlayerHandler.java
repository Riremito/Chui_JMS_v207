...
public class PlayerHandler {
    ...
    public static void ChangeSkillMacro(final InPacket inPacket, final MapleCharacter chr) {
        final int num = inPacket.decodeByte();
        String name;
        int shout, skill1, skill2, skill3;
        SkillMacro macro;

        for (int i = 0; i < num; i++) {
            name = inPacket.decodeString();
            shout = inPacket.decodeByte();
            skill1 = inPacket.decodeInt();
            skill2 = inPacket.decodeInt();
            skill3 = inPacket.decodeInt();
    ...
    public static final void ChangeKeymap(final InPacket inPacket, final MapleCharacter chr) {
        if (inPacket.getUnreadAmount() > 8 && chr != null) { // else = pet auto pot
            inPacket.skip(4); //0
            final int numChanges = inPacket.decodeInt();

            for (int i = 0; i < numChanges; i++) {
                final int key = inPacket.decodeInt();
                final byte type = inPacket.decodeByte();
                final int action = inPacket.decodeInt();
            ...
            final int type = inPacket.decodeInt(), data = inPacket.decodeInt();
            switch (type) {
                case 1:
                    if (data <= 0) {
...
/*        if (GameConstants.isFishingMap(chr.getMapId()) && (!GameConstants.GMS || itemId == 3011000)) {
    ...
    public static final void TrockAddMap(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        final byte addrem = inPacket.decodeByte();
        final byte vip = inPacket.decodeByte();

        if (vip == 1) {
            if (addrem == 0) {
                chr.deleteFromRocks(inPacket.decodeInt());
            } else if (addrem == 1) {
            ...
            if (addrem == 0) {
                chr.deleteFromHyperRocks(inPacket.decodeInt());
            } else if (addrem == 1) {
                if (!FieldLimitType.VipRock.check(chr.getMap().getFieldLimit())) {
                    chr.addHyperRockMap();
                } else {
            ...
            if (addrem == 0) {
                chr.deleteFromRegRocks(inPacket.decodeInt());
            } else if (addrem == 1) {
                if (!FieldLimitType.VipRock.check(chr.getMap().getFieldLimit())) {
                    chr.addRegRockMap();
                } else {
        ...
        c.write(MTSCSPacket.getTrockRefresh(chr, vip, addrem == 3));
    }
    ...
    public static final void CharInfoRequest(final int objectid, final MapleClient c, final MapleCharacter chr) {
        ...
        if (player != null && !player.isClone()) {
            if (!player.isGM() || c.getPlayer().isGM()) {
                c.write(MaplePacketCreator.charInfo(player, c.getPlayer().getId() == objectid));
            }
        }
    }

    public static final void TakeDamage(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        //System.out.println(slea.toString());
        inPacket.skip(4); // randomized
        chr.updateTick(inPacket.decodeInt());
        final byte type = inPacket.decodeByte(); //-4 is mist, -3 and -2 are map damage.
        inPacket.skip(1); // Element - 0x00 = elementless, 0x01 = ice, 0x02 = fire, 0x03 = lightning
        ...
        if (type != -2 && type != -3 && type != -4) { // Not map damage
            monsteridfrom = inPacket.decodeInt();
            oid = inPacket.decodeInt();
            attacker = chr.getMap().getMonsterByOid(oid);
            direction = inPacket.decodeByte();
        ...
        if (chr.getStat().dodgeChance > 0 && Randomizer.nextInt(100) < chr.getStat().dodgeChance) {
            c.write(MaplePacketCreator.showSpecialEffect(21)); //lol
            return;
        }
        ...
        if (damage > 0) {
            ...
            if (inPacket.getUnreadAmount() == 3 || inPacket.getUnreadAmount() == 4) {
                byte level = inPacket.decodeByte();
                if (level > 0) {
                    final MobSkill skill = MobSkillFactory.getMobSkill(inPacket.decodeShort(), level);
                    if (skill != null) {
                        skill.applyEffect(chr, attacker, false);
                    }
                }
...
//        c.write(MaplePacketCreator.enableActions());
    ...
    public static final void CancelMech(final InPacket inPacket, final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
        int sourceid = inPacket.decodeInt();
        ...
        if (skill.isChargeSkill()) {
            ...
            chr.cancelEffect(skill.getEffect(inPacket.decodeByte()), false, -1);
        }
    }

    public static final void QuickSlot(final InPacket inPacket, final MapleCharacter chr) {
        final StringBuilder ret = new StringBuilder();
        for (int i = 0; i < 8; i++) { //really hacky way of doing it
            ret.append(inPacket.decodeString(1));
            inPacket.skip(3);
        }
        chr.getQuestNAdd(MapleQuest.getInstance(GameConstants.QUICK_SLOT)).setCustomData(ret.toString());
    }

    public static final void SkillEffect(final InPacket inPacket, final MapleCharacter chr) {
        ...
        final byte level = inPacket.decodeByte();
        final byte flags = inPacket.decodeByte();
        final byte speed = inPacket.decodeByte();
        final byte unk = inPacket.decodeByte(); // Added on v.82
        ...
        if (skilllevel_serv > 0 && skilllevel_serv == level && (skillId == 33101005 || skill.isChargeSkill())) {
            ...
            if (skillId == 33101005) {
                chr.setLinkMid(inPacket.decodeInt(), 0);
            }
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.skillEffect(chr, skillId, level, flags, speed, unk), false);
        }
    }

    public static final void SpecialMove(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || chr.hasBlockedInventory() || chr.getMap() == null || inPacket.getUnreadAmount() < 9) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        inPacket.skip(4); // Old X and Y
        ...
        int skillLevel = inPacket.decodeByte();
        ...
        switch (skillid) {
            ...
            case 9101020:
            case 31111003:
                final byte number_of_mobs = inPacket.decodeByte();
                inPacket.skip(3);
                for (int i = 0; i < number_of_mobs; i++) {
                    ...
                    if (mob != null) {
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, inPacket.decodeByte()), chr.getTruePosition());
                ...
                chr.getMap().broadcastMessage(chr, MaplePacketCreator.showBuffeffect(chr.getId(), skillid, 1, chr.getLevel(), skillLevel, inPacket.decodeByte()), chr.getTruePosition());
                c.write(MaplePacketCreator.enableActions());
                break;
            case 30001061: //capture
                ...
                if (mob != null) {
                    ...
                    if (success) {
                        ...
                        c.write(MaplePacketCreator.updateJaguar(chr));
                    } else {
            ...
            default:
                Point pos = null;
                if (inPacket.getUnreadAmount() == 5 || inPacket.getUnreadAmount() == 7) {
    ...
    public static final void closeRangeAttack(final InPacket inPacket, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        AttackInfo attack = DamageParse.parseDmgM(inPacket, chr);
        if (attack == null) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
    ...
    public static final void rangedAttack(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
        if (chr.hasBlockedInventory() || chr.getMap() == null) {
        ...
        AttackInfo attack = DamageParse.parseDmgR(inPacket, chr);
        if (attack == null) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
                ...
                if (chr.getJob() == 412 && bulletConsume > 0 && ipp.getQuantity() < MapleItemInformationProvider.getInstance().getSlotMax(projectile)) {
                    ...
                    if (chr.getTotalSkillLevel(expert) > 0) {
                        ...
                        if (eff.makeChanceResult()) {
                            ...
                            c.write(MaplePacketCreator.updateInventorySlot(MapleInventoryType.USE, ipp, false));
                            bulletConsume = 0; //regain a star after using
                            c.write(MaplePacketCreator.getInventoryStatus());
                        }
                    }
                }
                if (bulletConsume > 0) {
...
//		basedamage = ((effect.getMoneyCon() * 10) / 100) * effect.getProb(); // Not sure
    ...
    public static final void MagicDamage(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || chr.hasBlockedInventory() || chr.getMap() == null) {
            return;
        }
        AttackInfo attack = DamageParse.parseDmgMa(inPacket, chr);
        if (attack == null) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
    ...
    public static final void MoveAndroid(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        final Point oldPos = inPacket.decodePosition();
        final Point oldVPos = inPacket.decodePosition();
        final List<ILifeMovementFragment> res = MovementParse.parseMovement(inPacket, oldPos, MovementKind.ANDROID_MOVEMENT);
        if (res != null && chr != null && res.size() != 0 && chr.getMap() != null && chr.getAndroid() != null) { // map crash hack
            ...
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.moveAndroid(chr.getId(), oldPos, oldVPos, res), false);
        }
    }
    ...
    public static final void Heal(final InPacket inPacket, final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
        chr.updateTick(inPacket.decodeInt());
        if (inPacket.getUnreadAmount() >= 8) {
            inPacket.skip(inPacket.getUnreadAmount() >= 12 ? 8 : 4);
        }
        int healHP = inPacket.decodeShort();
        int healMP = inPacket.decodeShort();
    ...
    public static final void MovePlayer(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null) {
            return;
        }
        inPacket.decodeLong(); //  FF FF FF FF FF FF FF FF
        inPacket.decodeByte(); // portal count
        inPacket.decodeLong(); //  FF FF FF FF FF FF FF FF
        inPacket.skip(16);
        final Point oldPos = inPacket.decodePosition();
        final Point oldVPos = inPacket.decodePosition();
        ...
        List<ILifeMovementFragment> res;
        try {
            res = MovementParse.parseMovement(inPacket, oldPos, MovementKind.PLAYER_MOVEMENT);
...
/*            if (inPacket.getUnreadAmount() < 11 || inPacket.getUnreadAmount() > 26) {
            ...
            if (chr.isHidden()) {
                chr.setLastRes(res);
                c.getPlayer().getMap().broadcastGMMessage(chr, MaplePacketCreator.movePlayer(chr.getId(), res, oldPos, oldVPos), false);
            } else {
                c.getPlayer().getMap().broadcastMessage(c.getPlayer(), MaplePacketCreator.movePlayer(chr.getId(), res, oldPos, oldVPos), false);
            }
            ...
            if (chr.getFollowId() > 0 && chr.isFollowOn() && chr.isFollowInitiator()) {
                ...
                if (fol != null) {
                    ...
                    fol.getClient().write(MaplePacketCreator.moveFollow(oldPos, oldVPos, pos, res));
                    MovementParse.updatePosition(res, fol, 0);
                    map.movePlayer(fol, pos);
                    map.broadcastMessage(fol, MaplePacketCreator.movePlayer(fol.getId(), res, oldPos, oldVPos), false);
                } else {
                    chr.checkFollow();
                }
            }
                    ...
                    CloneTimer.getInstance().schedule(new Runnable() {
                        ...
                        public void run() {
                            try {
                                if (clone.getMap() == map) {
                                    if (clone.isHidden()) {
                                        map.broadcastGMMessage(clone, MaplePacketCreator.movePlayer(clone.getId(), res3, oldPos, oldVPos), false);
                                    } else {
                                        map.broadcastMessage(clone, MaplePacketCreator.movePlayer(clone.getId(), res3, oldPos, oldVPos), false);
                                    }
...
//	inPacket.skip(2);
    ...
    public static final void ChangeMap(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || chr.getMap() == null) {
            return;
        }
        if (inPacket.getUnreadAmount() != 0) {
            //inPacket.skip(6); //D3 75 00 00 00 00
            inPacket.decodeByte(); // 1 = from dying 2 = regular portals
            int targetid = inPacket.decodeInt(); // FF FF FF FF
            final MaplePortal portal = chr.getMap().getPortal(inPacket.decodeString());
            if (inPacket.getUnreadAmount() >= 7) {
                chr.updateTick(inPacket.decodeInt());
            }
            inPacket.skip(1);
            final boolean wheel = inPacket.decodeShort() > 0 && !GameConstants.isEventMap(chr.getMapId()) && chr.haveItem(5510000, 1, false, true) && chr.getMapId() / 1000000 != 925;
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (!wheel) {
                    ...
                    c.write(MTSCSPacket.useWheel((byte) (chr.getInventory(MapleInventoryType.CASH).countById(5510000) - 1)));
    ...
    public static final void InnerPortal(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || chr.getMap() == null) {
            return;
        }
        final MaplePortal portal = chr.getMap().getPortal(inPacket.decodeString());
        final int toX = inPacket.decodeShort();
        final int toY = inPacket.decodeShort();
//	inPacket.decodeShort(); // Original X pos
//	inPacket.decodeShort(); // Original Y pos
    ...
    public static final void snowBall(InPacket inPacket, MapleClient c) {
        //B2 00
        //01 [team]
        //00 00 [unknown]
        //89 [position]
    ...
    public static final void leftKnockBack(InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().getMapId() / 10000 == 10906) { //must be in snowball map or else its like infinite FJ
            c.write(MaplePacketCreator.leftKnockBack());
            c.write(MaplePacketCreator.enableActions());
        }
    }

    public static final void ReIssueMedal(InPacket inPacket, MapleClient c, MapleCharacter chr) {
        final MapleQuest q = MapleQuest.getInstance(inPacket.decodeShort());
        if (q != null && q.getMedalItem() > 0 && chr.getQuestStatus(q.getId()) == 2 && !chr.haveItem(q.getMedalItem(), 1, true, true) && q.getMedalItem() == inPacket.decodeInt() && MapleInventoryManipulator.checkSpace(c, q.getMedalItem(), (short) 1, "")) {
