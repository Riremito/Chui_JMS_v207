...
public class PetHandler {
    ...
    public static final void SpawnPet(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        chr.updateTick(inPacket.decodeInt());
        chr.spawnPet(inPacket.decodeByte(), inPacket.decodeByte() > 0);

    }

    public static final void Pet_AutoPotion(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        inPacket.skip(GameConstants.GMS ? 9 : 1);
	    chr.updateTick(inPacket.decodeInt());
        ...
        if (toUse == null || toUse.getQuantity() < 1 || toUse.getItemId() != inPacket.decodeInt()) {
	...
	if (petCommand == null) {
        ...
        if (Randomizer.nextInt(99) <= petCommand.getProbability()) {
            ...
            if (pet.getCloseness() < 30000) {
                ...
                c.write(PetPacket.updatePet(pet, chr.getInventory(MapleInventoryType.CASH).getItem((byte) pet.getInventoryPosition()), true));
            }
        }
        chr.getMap().broadcastMessage(PetPacket.commandResponse(chr.getId(), (byte)petCommand.getSkillId(), petIndex, success, false));
    }

    public static final void PetFood(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void MovePet(final InPacket inPacket, final MapleCharacter chr) {
        final int petId = GameConstants.GMS ? ((int)inPacket.decodeLong()) : inPacket.decodeInt();
        
        final Point oldPos = inPacket.decodePosition();
        final Point oldVPos = inPacket.decodePosition();
        final List<ILifeMovementFragment> res = MovementParse.parseMovement(inPacket, oldPos, MovementKind.PET_MOVEMENT);
        ...
        if (res != null && chr != null && res.size() != 0 && chr.getMap() != null) { // map crash hack
            ...
            chr.getMap().broadcastMessage(chr, PetPacket.movePet(chr.getId(), pet.getUniqueId(), oldPos, oldVPos, (byte) petId, res), false);
