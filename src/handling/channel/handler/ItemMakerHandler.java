...
public class ItemMakerHandler {
    ...
    public static final void ItemMaker(final InPacket inPacket, final MapleClient c) {
        //System.out.println(slea.toString()); //change?
        final int makerType = inPacket.decodeInt();

        switch (makerType) {
            case 1: { // Gem
                final int toCreate = inPacket.decodeInt();
                ...
                if (GameConstants.isGem(toCreate)) {
                    ...
                    final boolean stimulator = inPacket.decodeByte() > 0;
                    final int numEnchanter = inPacket.decodeInt();
                    ...
                    if (stimulator || numEnchanter > 0) {
                        ...
                        for (int i = 0; i < numEnchanter; i++) {
                            final int enchant = inPacket.decodeInt();
            ...
            case 3: { // Making Crystals
                final int etc = inPacket.decodeInt();
    ...
    public static final void UseRecipe(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
    ...
    public static final void MakeExtractor(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMap() == null || chr.hasBlockedInventory()) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        final int itemId = inPacket.decodeInt();
        final int fee = inPacket.decodeInt();
    ...
    public static final void UseBag(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMap() == null || chr.hasBlockedInventory()) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        if (firstTime) {
            ...
            toUse.setFlag(flag);
            c.write(MaplePacketCreator.updateSpecialItemUse(toUse, (byte) 4, toUse.getPosition(), true, chr));
        }
        c.write(MaplePacketCreator.openBag(chr.getExtendedSlots().indexOf(itemId), itemId, firstTime));
        c.write(MaplePacketCreator.enableActions());
    }

    public static final void StartHarvest(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        //its ok if a hacker bypasses this as we do everything in the reactor anyway
        final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(inPacket.decodeInt());
...
//            c.getPlayer().dropMessage(5, "You may not harvest yet.");
            ...
            c.write(MaplePacketCreator.harvestMessage(reactor.getObjectId(), GameConstants.GMS ? 13 : 11)); //ok to harvest, gogo
            c.getPlayer().getMap().broadcastMessage(chr, MaplePacketCreator.showHarvesting(chr.getId(), item.getItemId()), false);
        }
    }

    public static final void StopHarvest(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        //its ok if a hacker bypasses this as we do everything in the reactor anyway
        /*final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(inPacket.decodeInt());
    ...
    public static final void ProfessionInfo(final InPacket inPacket, final MapleClient c) { //so pointless
		try {
			String asdf = inPacket.decodeString();
			int level1 = inPacket.decodeInt();
			c.write(MaplePacketCreator.professionInfo(asdf, level1, inPacket.decodeInt(), Math.max(0, 100 - ((level1 + 1) - c.getPlayer().getProfessionLevel(Integer.parseInt(asdf))) * 20)));
		} catch (NumberFormatException nfe) {
		} //idc
    }

    public static final void CraftEffect(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr.getMapId() != 910001000 && chr.getMap().getExtractorSize() <= 0) {
            return; //ardent mill
        }
        final String effect = inPacket.decodeString();
        ...
        if (profession != null && (c.getPlayer().getProfessionLevel(profession.intValue()) > 0 || (profession == 92040000 && chr.getMap().getExtractorSize() > 0))) {
            ...
            c.write(MaplePacketCreator.showOwnCraftingEffect(effect, time, effect.endsWith("Extract") ? 1 : 0));
    ...
    public static final void CraftMake(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr.getMapId() != 910001000 && chr.getMap().getExtractorSize() <= 0) {
            return; //ardent mill
        }
        final int something = inPacket.decodeInt(); // 92010002
        ...
        String effect = "";
        int actionid = 0;
        switch (something / 10000) {
            case 9200:
                effect = "Effect/BasicEff.img/professions/herbalismExtract";
                actionid = 282;
                break;
            case 9201:
                effect = "Effect/BasicEff.img/professions/miningExtract";
                actionid = 283;
                break;
            case 9202:
                effect = "Effect/BasicEff.img/professions/equip_product";
                actionid = 284;
                break;
            case 9204:
                effect = "Effect/BasicEff.img/professions/alchemy";
                actionid = 290;
                break;
            default:
                break;
        ...
        chr.getMap().broadcastMessage(MaplePacketCreator.craftMake(chr.getId(), actionid/*something*/, time));
        //}
    }

    public static final void CraftComplete(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        final int craftID = inPacket.decodeInt();
        ...
        if (craftID == 92049000) { //disassembling
            final int extractorId = inPacket.decodeInt();
            final int itemId = inPacket.decodeInt();
            final long invId = inPacket.decodeLong();
            ...
            final int itemId = inPacket.decodeInt();
            final long invId1 = inPacket.decodeLong();
            final long invId2 = inPacket.decodeLong();
            final int reqLevel = ii.getReqLevel(itemId);
        ...
        if (expGain > 0 && theLevl < 10) {
            ...
            switch (craftID / 10000) {
                ...
                case 9202:
                    s = "装備制作";
                    break;
                case 9203: // 飾品製作在日版跟裝備是合在一起的
                    s = "Accessory Crafting";
                    break;
            }
            chr.dropMessage(-5, s + "の熟練度が増加した。(+" + expGain + ")");
            if (chr.addProfessionExp((craftID / 10000) * 10000, expGain)) {
                chr.dropMessage(-5, s + "のレベルが上がりました。");
            }
        } else {
            expGain = 0;
        }
...
//        chr.setFatigue((byte) (chr.getFatigue() + fatigue));
    ...
    public static final void UsePot(final InPacket inPacket, final MapleClient c) {
        final int itemid = inPacket.decodeInt();
        final Item slot = c.getPlayer().getInventory(MapleInventoryType.USE).getItem(inPacket.decodeShort());
        ...
        for (int i = 0; i < c.getPlayer().getImps().length; i++) {
            if (c.getPlayer().getImps()[i] == null) {
                ...
                c.write(MaplePacketCreator.updateImp(c.getPlayer().getImps()[i], ImpFlag.SUMMONED.getValue(), i, false));
    ...
    public static final void ClearPot(final InPacket inPacket, final MapleClient c) {
        final int index = inPacket.decodeInt() - 1;
        if (index < 0 || index >= c.getPlayer().getImps().length || c.getPlayer().getImps()[index] == null) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        c.write(MaplePacketCreator.updateImp(c.getPlayer().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
        c.getPlayer().getImps()[index] = null;
    }

    public static final void FeedPot(final InPacket inPacket, final MapleClient c) {
        final int itemid = inPacket.decodeInt();
        final Item slot = c.getPlayer().getInventory(GameConstants.getInventoryType(itemid)).getItem((short) inPacket.decodeInt());
        ...
        c.write(MaplePacketCreator.updateImp(c.getPlayer().getImps()[index], mask, index, false));
    }

    public static final void CurePot(final InPacket inPacket, final MapleClient c) {
        final int itemid = inPacket.decodeInt();
        final Item slot = c.getPlayer().getInventory(MapleInventoryType.ETC).getItem((short) inPacket.decodeInt());
        ...
        c.getPlayer().getImps()[index].setState(1);
        c.write(MaplePacketCreator.updateImp(c.getPlayer().getImps()[index], ImpFlag.STATE.getValue(), index, false));
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.ETC, slot.getPosition(), (short) 1, false, false);
    }

    public static final void RewardPot(final InPacket inPacket, final MapleClient c) {
        final int index = inPacket.decodeInt() - 1;
