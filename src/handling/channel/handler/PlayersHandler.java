...
public class PlayersHandler {
    ...
    public static void Note(final InPacket inPacket, final MapleCharacter chr) {
        ...
        switch (type) {
            case 0:
                ...
                boolean fame = inPacket.decodeByte() > 0;
                inPacket.decodeInt(); //0?
                Item itemz = chr.getCashInventory().findByCashId((int) inPacket.decodeLong());
            ...
            case 1:
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    chr.deleteNote(id, inPacket.decodeByte() > 0 ? 1 : 0);
                }
                break;
            default:
    ...
    public static void GiveFame(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        final int who = inPacket.decodeInt();
        final int mode = inPacket.decodeByte();
    ...
    public static void UseDoor(final InPacket inPacket, final MapleCharacter chr) {
        final int oid = inPacket.decodeInt();
        final boolean mode = inPacket.decodeByte() == 0; // specifies if backwarp or not, 1 town to target, 0 target to town
    ...
    public static void UseMechDoor(final InPacket inPacket, final MapleCharacter chr) {
        final int oid = inPacket.decodeInt();
        final Point pos = inPacket.decodePosition();
        final int mode = inPacket.decodeByte(); // specifies if backwarp or not, 1 town to target, 0 target to town
    ...
    public static void TransformPlayer(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        // D9 A4 FD 00
        // 11 00
        // A0 C0 21 00
        ...
        final String target = inPacket.decodeString();
    ...
    public static void HitReactor(final InPacket inPacket, final MapleClient c) {
        final int oid = inPacket.decodeInt();
        final int charPos = inPacket.decodeInt();
        final short stance = inPacket.decodeShort();
    ...
    public static void TouchReactor(final InPacket inPacket, final MapleClient c) {
        final int oid = inPacket.decodeInt();
        final boolean touched = inPacket.getUnreadAmount() == 0 || inPacket.decodeByte() > 0; //the byte is probably the state to set it to
        final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(oid);
    ...
    public static void hitCoconut(InPacket inPacket, MapleClient c) {
        /*CB 00 A6 00 06 01
         * A6 00 = coconut id
         * 06 01 = ?
         */
        int id = inPacket.decodeShort();
        String co = "coconut";
    ...
    public static void FollowRequest(final InPacket inPacket, final MapleClient c) {
        MapleCharacter tt = c.getPlayer().getMap().getCharacterById(inPacket.decodeInt());
        ...
        if (inPacket.decodeByte() > 0) { //cancelling follow
            tt = c.getPlayer().getMap().getCharacterById(c.getPlayer().getFollowId());
        ...
        if (tt != null && tt.getPosition().distanceSq(c.getPlayer().getPosition()) < 10000 && tt.getFollowId() == 0 && c.getPlayer().getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) { //estimate, should less
            ...
            tt.getClient().write(MaplePacketCreator.followRequest(c.getPlayer().getId()));
        } else {
            c.write(MaplePacketCreator.serverNotice(1, "You are too far away."));
        }
    }

    public static void FollowReply(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().getFollowId() > 0 && c.getPlayer().getFollowId() == inPacket.decodeInt()) {
            ...
            if (tt != null && tt.getPosition().distanceSq(c.getPlayer().getPosition()) < 10000 && tt.getFollowId() == 0 && tt.getId() != c.getPlayer().getId()) { //estimate, should less
                ...
                if (accepted) {
                    ...
                    tt.setFollowId(0);
                    tt.getClient().write(MaplePacketCreator.getFollowMsg(5));
                }
            } else {
                if (tt != null) {
                    tt.setFollowId(0);
    ...
    public static void DoRing(final MapleClient c, final String name, final int itemid) {
        ...
        if (errcode > 0) {
            c.write(MaplePacketCreator.sendEngagement((byte) errcode, 0, null, null));
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        c.getPlayer().setMarriageItemId(itemid);
        chr.getClient().write(MaplePacketCreator.sendEngagementRequest(c.getPlayer().getName(), c.getPlayer().getId()));
    }

    public static void RingAction(final InPacket inPacket, final MapleClient c) {
        final byte mode = inPacket.decodeByte();
        if (mode == 0) {
            DoRing(c, inPacket.decodeString(), inPacket.decodeInt());
            //1112300 + (itemid - 2240004)
        } else if (mode == 1) {
            c.getPlayer().setMarriageItemId(0);
        } else if (mode == 2) { //accept/deny proposal
            final boolean accepted = inPacket.decodeByte() > 0;
            ...
            if (c.getPlayer().getMarriageId() > 0 || chr == null || chr.getId() != id || chr.getMarriageItemId() <= 0 || !chr.haveItem(chr.getMarriageItemId(), 1) || chr.getMarriageId() > 0 || !chr.isAlive() || chr.getEventInstance() != null || !c.getPlayer().isAlive() || c.getPlayer().getEventInstance() != null) {
                c.write(MaplePacketCreator.sendEngagement((byte) 0x1D, 0, null, null));
                c.write(MaplePacketCreator.enableActions());
                return;
            }
            if (accepted) {
                ...
                if (!MapleInventoryManipulator.checkSpace(c, newItemId, 1, "") || !MapleInventoryManipulator.checkSpace(chr.getClient(), newItemId, 1, "")) {
                    c.write(MaplePacketCreator.sendEngagement((byte) 0x15, 0, null, null));
                    c.write(MaplePacketCreator.enableActions());
                    return;
                }
                try {
                    ...
                    chr.getClient().write(MaplePacketCreator.sendEngagement((byte) 0x10, newItemId, chr, c.getPlayer()));
                ...
                chr.getClient().write(MaplePacketCreator.sendEngagement((byte) 0x1E, 0, null, null));
            }
            c.write(MaplePacketCreator.enableActions());
    ...
    public static void Solomon(final InPacket inPacket, final MapleClient c) {
        c.write(MaplePacketCreator.enableActions());
        c.getPlayer().updateTick(inPacket.decodeInt());
        Item item = c.getPlayer().getInventory(MapleInventoryType.USE).getItem(inPacket.decodeShort());
        if (item == null || item.getItemId() != inPacket.decodeInt() || item.getQuantity() <= 0 || c.getPlayer().getGachExp() > 0 || c.getPlayer().getLevel() > 50 || MapleItemInformationProvider.getInstance().getItemEffect(item.getItemId()).getEXP() <= 0) {
            return;
        }
...
//        c.getPlayer().updateSingleStat(MapleStat.GACHAPONEXP, c.getPlayer().getGachExp());
    ...
    public static void GachExp(final InPacket inPacket, final MapleClient c) {
        c.write(MaplePacketCreator.enableActions());
        c.getPlayer().updateTick(inPacket.decodeInt());
...
//        c.getPlayer().updateSingleStat(MapleStat.GACHAPONEXP, 0);
    ...
    public static void Report(final InPacket inPacket, final MapleClient c) {
        //0 = success 1 = unable to locate 2 = once a day 3 = you've been reported 4+ = unknown reason
        MapleCharacter other;
        ReportType type;
        if (!GameConstants.GMS) {
            other = c.getPlayer().getMap().getCharacterById(inPacket.decodeInt());
            type = ReportType.getById(inPacket.decodeByte());
        } else {
            type = ReportType.getById(inPacket.decodeByte());
            other = c.getPlayer().getMap().getCharacterByName(inPacket.decodeString());
            //then,byte(?) and string(reason)
        }
        if (other == null || type == null || other.isIntern()) {
            c.write(MaplePacketCreator.report(4));
            return;
        }
        ...
        if (theTime + 7200000 > currentTime && !c.getPlayer().isIntern()) {
            ...
            c.write(MaplePacketCreator.report(GameConstants.GMS ? 2 : 0));
        }
    }

    public static final void MonsterBookInfoRequest(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null) {
            return;
        }
        inPacket.decodeInt(); //no clue
        final MapleCharacter player = c.getPlayer().getMap().getCharacterById(inPacket.decodeInt());
        c.write(MaplePacketCreator.enableActions());
        if (player != null && !player.isClone()) {
            if (!player.isGM() || c.getPlayer().isGM()) {
                c.write(MaplePacketCreator.getMonsterBookInfo(player));
            }
        }
    }

    public static final void ChangeSet(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null) {
            return;
        }
        final int set = inPacket.decodeInt();
        if (chr.getMonsterBook().changeSet(set)) {
            ...
            c.write(MaplePacketCreator.changeCardSet(set));
        }
    }

    public static final void EnterPVP(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || c.getPlayer().getMapId() != 960000000) {
            c.write(MaplePacketCreator.pvpBlocked(1));
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        if (c.getPlayer().getParty() != null) {
            c.write(MaplePacketCreator.pvpBlocked(9));
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        c.getPlayer().updateTick(inPacket.decodeInt());
        inPacket.skip(1);
        int type = inPacket.decodeByte(), lvl = inPacket.decodeByte(), playerCount = 0;
        boolean passed = switch (lvl) {
            case 0 -> c.getPlayer().getLevel() >= 30 && c.getPlayer().getLevel() < 70;
            case 1 -> c.getPlayer().getLevel() >= 70;
            case 2 -> c.getPlayer().getLevel() >= 120;
            case 3 -> c.getPlayer().getLevel() >= 180;
            default -> false;
        };
        ...
        for (int i : maps) {
            final EventInstanceManager eim = em.getInstance(new StringBuilder("PVP").append(key).append(i).toString().replace(" ", "").replace(" ", ""));
    ...
    public static final void RespawnPVP(/*final InPacket inPacket, */final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || !c.getPlayer().inPVP() || c.getPlayer().isAlive()) {
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        EventInstanceManager eim = c.getPlayer().getEventInstance();
        if (eim.getName().startsWith("PVP")) {
            final int type = Integer.parseInt(eim.getProperty("type"));
            ...
            final int lvl = Integer.parseInt(eim.getProperty("lvl"));
//            c.getPlayer().getStat().heal_noUpdate(c.getPlayer());
//            c.getPlayer().updateSingleStat(MapleStat.MP, c.getPlayer().getStat().getMp());
//            c.getPlayer().getStat().recalcLocalStats(true, c.getPlayer());
            final Lock ThreadLock = new ReentrantLock();
            ThreadLock.lock();
            try {
                eim.schedule("updateScoreboard", 500);
            } finally {
                ThreadLock.unlock();
            }
//            c.getPlayer().changeMap(c.getPlayer().getMap(), c.getPlayer().getMap().getPortal(type == 0 ? 0 : (type == 3 ? (c.getPlayer().getTeam() == 0 ? 3 : 1) : (c.getPlayer().getTeam() == 0 ? 2 : 3))));
            c.getPlayer().getStat().recalcLocalStats(false, c.getPlayer());
            c.getPlayer().getStat().heal(c.getPlayer());
            c.write(MaplePacketCreator.getPVPScore(Integer.parseInt(eim.getProperty(String.valueOf(c.getPlayer().getId()))), false));
            List<Pair<Integer, String>> chr = new ArrayList<Pair<Integer, String>>();
            for (int xx = 0; xx < eim.getPlayers().size(); xx++) {
                if (eim.getPlayers().get(xx).getTeam() == c.getPlayer().getTeam()) {
                    chr.add(new Pair<Integer, String>(eim.getPlayers().get(xx).getId(), eim.getPlayers().get(xx).getName()));
                }
            }
            c.write(MaplePacketCreator.getPVPType(type, chr, c.getPlayer().getTeam() + 1, !eim.getProperty("started").equals("0"), lvl));
            c.write(MaplePacketCreator.getPVPTeam(chr));
            c.write(MaplePacketCreator.enablePVP(true));
        }
    }
	
    public static final void LeavePVP(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer() == null || c.getPlayer().getMap() == null || !c.getPlayer().inPVP()) {
            c.write(MaplePacketCreator.pvpBlocked(6));
            c.write(MaplePacketCreator.enableActions());
            return;
        }
        ...
        c.getPlayer().clearAllCooldowns();
        c.getPlayer().updateTick(inPacket.decodeInt());
        c.write(UIPacket.clearMidMsg());
        c.getPlayer().changeMap(c.getChannelServer().getMapFactory().getMap(960000000));
        c.getPlayer().getStat().recalcLocalStats(c.getPlayer());
        c.getPlayer().getStat().heal(c.getPlayer());
    }

    public static final void AttackPVP(final InPacket inPacket, final MapleClient c) {
        final MapleCharacter chr = c.getPlayer();
        final int trueSkill = inPacket.decodeInt();
        ...
        inPacket.decodeByte(); // 技能等級
        int chargeTime = 0;
        if (GameConstants.isMagicChargeSkill(skillid)) {
            chargeTime = inPacket.decodeInt();
        } else {
            inPacket.skip(4);
        }
        boolean facingLeft = inPacket.decodeByte() > 0;
        if (skillid > 0) {
            ...
            System.out.println("animation1: " + animation);
            if (animation == -1 && !skil.isMagic()) {
                    ...
                    if (p != null) {
                        while (animation == -1) {
                            ...
                            if (SkillFactory.getDelay(ep.left) != null) {
                                ...
                                System.out.println("animation2: " + animation);
                            }
                        }
                    }
                }
                ...
                System.out.println("animation3: " + animation);
            }
            if (skil.isMagic()) {
            ...
            if (after != null) {
                ...
                if (p != null) {
                    while (animation == -1) {
                        ...
                        if (SkillFactory.getDelay(ep.left) != null) {
                            animation = SkillFactory.getDelay(ep.left);
                            System.out.println("animation4: " + animation);
                            lt = ep.mid;
                            rb = ep.right;
                        }
                    }
        ...
        final int weapontype = inPacket.decodeByte();
        inPacket.skip(3); //?idk
        final int speed = inPacket.decodeByte();
        final int slot = inPacket.decodeShort();
        final int csstar = inPacket.decodeShort();
        int visProjectile = 0;
            ...
            for (MapleCharacter attacked : chr.getMap().getCharactersIntersect(box)) {
                if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden() && (type == 0 || attacked.getTeam() != chr.getTeam())) {
                    ...
                    if (ice == attacked.getId()) {
                        chr.getClient().write(MaplePacketCreator.getPVPIceHPBar(attacked.getStat().getHp(), attacked.getStat().getCurrentMaxHp()));
                    } else {
                        chr.getClient().write(MaplePacketCreator.getPVPHPBar(attacked.getId(), attacked.getStat().getHp(), attacked.getStat().getCurrentMaxHp()));
                    }
        ...
        if (killed || addedScore > 0) {
            ...
            chr.getClient().write(MaplePacketCreator.getPVPScore(ourScore + addedScore, killed));
        }
        if (didAttack) {
        ...
        if (weapontype == 7 && animation < 30) { // 拳套
            animation = Randomizer.rand(33, 35);
            System.out.println("animation5: " + animation);
        }
