...
public class PartyHandler {
    ...
    public static final void DenyPartyRequest(final InPacket inPacket, final MapleClient c) {
        final int action = inPacket.decodeByte();
        if (action == 0x32 && GameConstants.GMS) { //TODO JUMP
            final MapleCharacter chr = c.getPlayer().getMap().getCharacterById(inPacket.decodeInt());
    ...
    public static final void PartyOperation(final InPacket inPacket, final MapleClient c) {
        final int operation = inPacket.decodeByte();
        ...
        switch (operation) {
            case 1: // create
                if (party == null) {
                    ...
                    c.getPlayer().setParty(party);
                    c.write(MaplePacketCreator.partyCreated(party.getId()));
            ...
            case 4: // invite
                ...
                final String theName = inPacket.decodeString();
            ...
            case 5: // expel
                if (party != null && partyplayer != null && partyplayer.equals(party.getLeader())) {
                    ...
                    final MaplePartyCharacter expelled = party.getMemberById(inPacket.decodeInt());
                    if (expelled != null) {
            ...
            case 6: // change leader
                if (party != null) {
                    ...
                    final MaplePartyCharacter newleader = party.getMemberById(inPacket.decodeInt());
            ...
            case 7: //request to  join a party
                ...
                final int partyid_ = inPacket.decodeInt();
                if (GameConstants.GMS) {
                    ...
                    if (party != null && party.getMembers().size() < 6) {
                        ...
                        if (cfrom != null && cfrom.getQuestNoAdd(MapleQuest.getInstance(GameConstants.PARTY_REQUEST)) == null) {
                            c.write(MaplePacketCreator.partyStatusMessage(50, c.getPlayer().getName()));
                            cfrom.getClient().write(MaplePacketCreator.partyRequestInvite(c.getPlayer()));
                        } else {
    ...
    public static final void AllowPartyInvite(final InPacket inPacket, final MapleClient c) {
        if (inPacket.decodeByte() > 0) {
    ...
    public static final void DenySidekickRequest(final InPacket inPacket, final MapleClient c) {
        final int action = inPacket.decodeByte();
        final int cid = inPacket.decodeInt();
        if (c.getPlayer().getSidekick() == null && action == 0x5A) { //accept
            ...
            if (party != null) {
                ...
                if (sid <= 0) {
                    ...
                    c.write(MaplePacketCreator.updateSidekick(c.getPlayer(), s, true));
                    party.setSidekick(s);
                    party.getClient().write(MaplePacketCreator.updateSidekick(party, s, true));
                }
            } else {
    ...
    public static final void SidekickOperation(final InPacket inPacket, final MapleClient c) {
        final int operation = inPacket.decodeByte();

        switch (operation) {
            case 0x41: // create
                if (c.getPlayer().getSidekick() == null) {
                    final MapleCharacter other = c.getPlayer().getMap().getCharacterByName(inPacket.decodeString());
                    if (other.getSidekick() == null && MapleSidekick.checkLevels(c.getPlayer().getLevel(), other.getLevel())) {
                        other.getClient().write(MaplePacketCreator.sidekickInvite(c.getPlayer()));
    ...
    public static final void PartySearchStart(final InPacket inPacket, final MapleClient c) {
        if (c.getPlayer().isInBlockedMap() || FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())) {
            ...
            c.write(MaplePacketCreator.showMemberSearch(c.getPlayer().getMap().getCharactersThreadsafe()));
            return;
        ...
        final int min = inPacket.decodeInt();
        final int max = inPacket.decodeInt();
        final int members = inPacket.decodeInt();
        final int jobs = inPacket.decodeInt();
    ...
    public static final void PartySearchStop(final InPacket inPacket, final MapleClient c) {
        if (GameConstants.GMS) {
            ...
            c.write(MaplePacketCreator.showPartySearch(parties));
        }
    }

    public static final void PartyListing(final InPacket inPacket, final MapleClient c) {
        ...
        switch (mode) {
            ...
            case -97:
            case -105:
                pst = PartySearchType.getById(inPacket.decodeInt());
                ...
                if (c.getPlayer().getParty() == null && World.Party.searchParty(pst).size() < 10) {
                    ...
                    c.getPlayer().setParty(party);
                    c.write(MaplePacketCreator.partyCreated(party.getId()));
                    final PartySearch ps = new PartySearch(inPacket.decodeString(), pst.exped ? party.getExpeditionId() : party.getId(), pst);
                    World.Party.addSearch(ps);
                    if (pst.exped) {
                        c.write(MaplePacketCreator.expeditionStatus(World.Party.getExped(party.getExpeditionId()), true));
                    }
                    c.write(MaplePacketCreator.partyListingAdded(ps));
                } else {
            ...
            case -103:
                ...
                c.write(MaplePacketCreator.getPartyListing(pst));
                break;
            case 84: //close
            case 0xA2:
            case -94:
            ...
            case -101:
                ...
                if (party == null) { //are we in a party? o.O"
                    final int theId = inPacket.decodeInt();
                        ...
                        if (exped != null) {
                            ...
                            if (ps != null && c.getPlayer().getLevel() <= ps.getType().maxLevel && c.getPlayer().getLevel() >= ps.getType().minLevel && exped.getAllMembers() < exped.getType().maxMembers) {
                                ...
                                if (partyId < 0) {
                                    ...
                                    c.getPlayer().setParty(party);
                                    c.write(MaplePacketCreator.partyCreated(party.getId()));
                                    c.write(MaplePacketCreator.expeditionStatus(exped, true));
                                ...
                                c.write(MaplePacketCreator.expeditionError(0, c.getPlayer().getName()));
                            }
                        }
                    }
                }
    ...
    public static final void Expedition(final InPacket inPacket, final MapleClient c) {
        ...
        switch (mode) {
            ...
            case 119:
                final ExpeditionType et = ExpeditionType.getById(inPacket.decodeInt());
                if (et != null && c.getPlayer().getParty() == null && c.getPlayer().getLevel() <= et.maxLevel && c.getPlayer().getLevel() >= et.minLevel) {
                    ...
                    c.write(MaplePacketCreator.expeditionError(0, ""));
                }
                break;
                ...
                if (theCh > 0) {
                    ...
                    if (invited != null && invited.getParty() == null && party != null && party.getExpeditionId() > 0) {
                        ...
                        if (me != null && me.getAllMembers() < me.getType().maxMembers && invited.getLevel() <= me.getType().maxLevel && invited.getLevel() >= me.getType().minLevel) {
                            c.write(MaplePacketCreator.expeditionError(7, invited.getName()));
                            invited.getClient().write(MaplePacketCreator.expeditionInvite(c.getPlayer(), me.getType().exped));
                        } else {
                            c.write(MaplePacketCreator.expeditionError(3, invited.getName()));
                        }
                    } else {
                        c.write(MaplePacketCreator.expeditionError(2, name));
                    }
                } else {
                    c.write(MaplePacketCreator.expeditionError(0, name));
                }
                break;
                    ...
                    if (cfrom != null && cfrom.getParty() != null && cfrom.getParty().getExpeditionId() > 0) {
                        ...
                        if (exped != null && action == 8) {
                            if (c.getPlayer().getLevel() <= exped.getType().maxLevel && c.getPlayer().getLevel() >= exped.getType().minLevel && exped.getAllMembers() < exped.getType().maxMembers) {
                                ...
                                c.write(MaplePacketCreator.expeditionError(3, cfrom.getName()));
                            }
                        } else if (action == 9) {
                            cfrom.getClient().write(MaplePacketCreator.partyStatusMessage(23, c.getPlayer().getName()));
                        }
            ...
            case 124:
                ...
                if (part != null && part.getExpeditionId() > 0) {
                    final MapleExpedition exped = World.Party.getExped(part.getExpeditionId());
                    if (exped != null && exped.getLeader() == c.getPlayer().getId()) {
                        final MaplePartyCharacter newleader = part.getMemberById(inPacket.decodeInt());
                        if (newleader != null) {
            ...
            case 126:
                ...
                if (part != null && part.getExpeditionId() > 0) {
                    final MapleExpedition exped = World.Party.getExped(part.getExpeditionId());
                    if (exped != null && exped.getLeader() == c.getPlayer().getId()) {
                        final int partyIndexTo = inPacket.decodeInt();
                                    ...
                                    if (expelled != null && expelled.isOnline()) {
                                        ...
                                        if (partyIndexTo < exped.getParties().size()) { //already exists
                                            ...
                                            if (party != null && party.getMembers().size() < 6) {
                                                ...
                                                chr.receivePartyMemberHP();
                                                chr.updatePartyMemberHP();
                                                chr.getClient().write(MaplePacketCreator.expeditionStatus(exped, true));
                                            }
                                        } else {
                                            party = World.Party.createPartyAndAdd(expelled, exped.getId());
                                            chr.setParty(party);
                                            chr.getClient().write(MaplePacketCreator.partyCreated(party.getId()));
                                            chr.getClient().write(MaplePacketCreator.expeditionStatus(exped, true));
                                            World.Party.expedPacket(exped.getId(), MaplePacketCreator.expeditionUpdate(exped.getIndex(party.getId()), party), null);
                                        }
                                        if (c.getPlayer().getEventInstance() != null) {
