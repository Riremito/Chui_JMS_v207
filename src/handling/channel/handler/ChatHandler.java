...
public class ChatHandler {
    ...
    public static final void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
        if (text.length() > 0 && chr != null && chr.getMap() != null && !CommandProcessor.processCommand(c, text, chr.getBattle() == null ? CommandType.NORMAL : CommandType.POKEMON)) {
			...
			if (unk == 0) {
                ...
                c.write(MaplePacketCreator.serverNotice(6, "You have been muted and are therefore unable to talk."));
            }
        }
    }

    public static final void Others(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        final int type = inPacket.decodeByte();
        final byte numRecipients = inPacket.decodeByte();
	if (numRecipients <= 0) {
	    return;
	}
        ...
        for (byte i = 0; i < numRecipients; i++) {
            recipients[i] = inPacket.decodeInt();
        }
        final String chattext = inPacket.decodeString();
    ...
    public static final void Messenger(final InPacket inPacket, final MapleClient c) {
        String input;
        MapleMessenger messenger = c.getPlayer().getMessenger();

        switch (inPacket.decodeByte()) {
            case 0x00: // open
                if (messenger == null) {
                    int messengerid = inPacket.decodeInt();
            ...
            case 0x03: // invite
                ...
                if (messenger != null) {
                    ...
                    if (target != null) {
                        if (target.getMessenger() == null) {
                            ...
                            c.write(MaplePacketCreator.messengerChat(c.getPlayer().getName() + " : " + target.getName() + " is already using Maple Messenger."));
                        }
                    } else {
                ...
                break;
            case 0x05: // decline
                final String targeted = inPacket.decodeString();
    ...
    public static final void Whisper_Find(final InPacket inPacket, final MapleClient c) {
        final byte mode = inPacket.decodeByte();
        inPacket.decodeInt(); //ticks
        switch (mode) {
            case 68: //buddy
            case 5: { // Find

                final String recipient = inPacket.decodeString();
                MapleCharacter player = c.getChannelServer().getPlayerStorage().getCharacterByName(recipient);
                if (player != null) {
                    if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {

                        c.write(MaplePacketCreator.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 68));
                    } else {
			    ...
			    break;
                        ...
                        if (player != null) {
                            if (!player.isIntern() || (c.getPlayer().isIntern() && player.isIntern())) {
                                c.write(MaplePacketCreator.getFindReply(recipient, (byte) ch, mode == 68));
                            } else {
                                c.write(MaplePacketCreator.getWhisperReply(recipient, (byte) 0));
                            }
                            return;
                    ...
                    if (ch == -10) {
                        c.write(MaplePacketCreator.getFindReplyWithCS(recipient, mode == 68));
                    } else if (ch == -20) {
		...
		if (c.getPlayer() == null || c.getPlayer().getMap() == null) {
			...
			break;
                    ...
                    player.getClient().write(MaplePacketCreator.getWhisper(c.getPlayer().getName(), c.getChannel(), text));
