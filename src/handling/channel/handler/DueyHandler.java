...
public class DueyHandler {
     ...
     */
    public static final void DueyOperation(final InPacket inPacket, final MapleClient c) {
        /*
        final byte operation = inPacket.decodeByte();

        switch (operation) {
        case 1: { // Start Duey, 13 digit AS
        final String AS13Digit = inPacket.decodeString();
        //		int unk = inPacket.decodeInt(); // Theres an int here, value = 1
        //  9 = error
        ...
        final byte inventId = inPacket.decodeByte();
        final short itemPos = inPacket.decodeShort();
        final short amount = inPacket.decodeShort();
        final int mesos = inPacket.decodeInt();
        final String recipient = inPacket.decodeString();
        boolean quickdelivery = inPacket.decodeByte() > 0;
        ...
        final int packageid = inPacket.decodeInt();
