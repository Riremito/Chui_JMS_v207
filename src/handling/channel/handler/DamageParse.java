...
public class DamageParse {
    ...
    public static void applyAttack(final AttackInfo attack, final Skill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
        ...
        if (attack.skill != 0) {
            ...
            } else if (GameConstants.isInflationSkill(attack.skill) || attack.skill == 1140) {
...
//          }
    ...
    public static final AttackInfo parseDmgMa(final InPacket inPacket, final MapleCharacter chr) {
        //System.out.println(lea.toString());
        final AttackInfo ret = new AttackInfo();

        inPacket.decodeByte();
        inPacket.decodeInt(); // 51 6F 40 88 或 FF FF FF FF
        inPacket.decodeInt(); // 1F 01 02 FA 或 FF FF FF FF
        ret.tbyte = inPacket.decodeByte();
        //System.out.println("TBYTE: " + tbyte);
        ret.targets = (byte) ((ret.tbyte >>> 4) & 0xF);
        ret.hits = (byte) (ret.tbyte & 0xF);
        inPacket.decodeInt(); // 17 10 70 E0 或 FF FF FF FF
        inPacket.decodeInt(); // FF FE FF FF 或 3F 0C 38 FE
        ret.skill = inPacket.decodeInt();
        if (ret.skill >= 91000000) { //guild/recipe? no
            return null;
        }
        inPacket.skip(13/*17*/); // ORDER [1] byte on bigbang, [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82

        if (GameConstants.isMagicChargeSkill(ret.skill)) {
            ret.charge = inPacket.decodeInt();
        } else {
            ret.charge = -1;
        }
        ret.unk = inPacket.decodeByte();
        ret.display = inPacket.decodeUShort();
        System.out.println("animation: " + ret.display);
        inPacket.skip(4); //big bang
        inPacket.skip(1); // Weapon class
        ret.speed = inPacket.decodeByte(); // Confirmed
        ret.lastAttackTickCount = inPacket.decodeInt(); // Ticks
        inPacket.skip(4); //0
        ...
        for (int i = 0; i < ret.targets; i++) {
            ...
            //    }
            //}
            inPacket.skip(14); // [1] Always 6?, [3] unk, [4] Pos1, [4] Pos2, [2] seems to change randomly for some attack

            allDamageNumbers = new ArrayList<Pair<Integer, Boolean>>();

            for (int j = 0; j < ret.hits; j++) {
                damage = inPacket.decodeInt();
                allDamageNumbers.add(new Pair<Integer, Boolean>(Integer.valueOf(damage), false));
            }
            inPacket.skip(4); // CRC of monster [Wz Editing]
            ret.allDamage.add(new AttackPair(Integer.valueOf(oid), allDamageNumbers));
        }
        ret.position = inPacket.decodePosition();

        return ret;
    }

    public static final AttackInfo parseDmgM(final InPacket inPacket, final MapleCharacter chr) {
        //System.out.println(lea.toString());
        final AttackInfo ret = new AttackInfo();
        ...
        inPacket.skip(13); // ORDER [1] byte on bigbang, [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82
        switch (ret.skill) {
        ...
        //    inPacket.skip(1); //1, 2, 3, or 4; which hit in the combo
        //}
        inPacket.skip(4); //big bang
        inPacket.skip(1); // Weapon class
        ret.speed = inPacket.decodeByte(); // Confirmed
        ...
        if (ret.skill == 4211006) { // Meso Explosion
            return parseMesoExplosion(inPacket, ret, chr);
        }
        int damage, oid;
        List<Pair<Integer, Boolean>> allDamageNumbers;
    ...
    public static final AttackInfo parseDmgR(final InPacket inPacket, final MapleCharacter chr) {
        ...
        inPacket.skip(14/*18*/); // ORDER [2] byte on bigbang [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82

        switch (ret.skill) { // Hurricane
            case 3121004, 3221001, 5221004, 13111002, 33121009, 35001001, 35101009, 23121000, 5311002 ->
                    inPacket.skip(4); // extra 4 bytes
        }
        ret.charge = -1;
        ret.unk = inPacket.decodeByte();
        ret.display = inPacket.decodeUShort();
        ...
        ret.lastAttackTickCount = inPacket.decodeInt(); // Ticks
        inPacket.skip(4); //0
        ret.slot = (byte) inPacket.decodeShort();
        ret.csstar = (byte) inPacket.decodeShort();
        ret.AOE = inPacket.decodeByte(); // is AOE or not, TT/ Avenger = 41, Showdown = 0
...
//	    System.out.println(tools.HexTool.toString(lea.read(4)));
    ...
    public static final AttackInfo parseMesoExplosion(final InPacket inPacket, final AttackInfo ret, final MapleCharacter chr) {
        //System.out.println(lea.toString(true));
        byte bullets;
        if (ret.hits == 0) {
            inPacket.skip(4);
            bullets = inPacket.decodeByte();
            for (int j = 0; j < bullets; j++) {
                ret.allDamage.add(new AttackPair(Integer.valueOf(inPacket.decodeInt()), null));
                inPacket.skip(1);
            }
            inPacket.skip(2); // 8F 02
            return ret;
        }
        int oid;
        List<Pair<Integer, Boolean>> allDamageNumbers;
        ...
        for (int i = 0; i < ret.targets; i++) {
            ...
            allDamageNumbers = new ArrayList<Pair<Integer, Boolean>>();
            for (int j = 0; j < bullets; j++) {
                allDamageNumbers.add(new Pair<Integer, Boolean>(Integer.valueOf(inPacket.decodeInt()), false)); //m.e. never crits
            }
            ret.allDamage.add(new AttackPair(Integer.valueOf(oid), allDamageNumbers));
            inPacket.skip(4); // C3 8F 41 94, 51 04 5B 01
        }
        inPacket.skip(4);
        bullets = inPacket.decodeByte();
