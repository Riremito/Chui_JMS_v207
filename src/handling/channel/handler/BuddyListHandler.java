...
public class BuddyListHandler {
    ...
    public static final void BuddyOperation(final InPacket inPacket, final MapleClient c) {
        final int mode = inPacket.decodeByte();
        final BuddyList buddylist = c.getPlayer().getBuddylist();

        if (mode == 1) { // add
            final String addName = inPacket.decodeString();
            final String groupName = inPacket.decodeString();
		...
		c.write(MaplePacketCreator.updateBuddylist(buddylist.getBuddies(), 10));
            ...
            int otherCid = inPacket.decodeInt();
            ...
            final int otherCid = inPacket.decodeInt();
	    final BuddylistEntry blz = buddylist.get(otherCid);
            ...
            c.write(MaplePacketCreator.updateBuddylist(buddylist.getBuddies(), 18));
	}
    }
