...
public class InterServerHandler {
    ...
    public static final void EnterCS(final MapleClient c, final MapleCharacter chr, final boolean mts) {
        if (chr.hasBlockedInventory() || chr.getMap() == null || chr.getEventInstance() != null || c.getChannelServer() == null) {
            c.write(MaplePacketCreator.serverBlocked(2));
...
//        c.write(MaplePacketCreator.getChannelChange(c, Integer.parseInt(CashShopServer.getIP().split(":")[1])));
        c.setPlayer(null);
        c.setReceiving(false);
        
        c.disconnect(true, false);
        final CharacterTransfer transfer = CashShopServer.getPlayerStorage().getPendingCharacter(chr.getId());
        CashShopOperation.EnterCS(chr.getId(), c, transfer);
    }
...
//        }
        ...
        c.write(MaplePacketCreator.temporaryStats_Reset()); // .
        ...
        try {
            ...
            if (player.getParty() != null) {
                ...
                if (party != null && party.getExpeditionId() > 0) {
                    ...
                    if (me != null) {
                        c.write(MaplePacketCreator.expeditionStatus(me, false));
                    }
                }
            }
            ...
            if (player.getSidekick() != null) {
                c.write(MaplePacketCreator.updateSidekick(player, player.getSidekick(), false));
            }
        ...
        player.getClient().write(MaplePacketCreator.serverMessage(channelServer.getServerMessage()));
        player.sendMacros();
        player.showNote();
        player.sendImp();
        ...
        c.write(MaplePacketCreator.TraitShowForeign(player.getId()));
        player.spawnClones();
        ...
        c.write(MaplePacketCreator.pendantSlot(stat != null && stat.getCustomData() != null && Long.parseLong(stat.getCustomData()) > System.currentTimeMillis()));
	    stat = player.getQuestNoAdd(MapleQuest.getInstance(GameConstants.QUICK_SLOT));
	    c.write(MaplePacketCreator.quickSlot(stat != null && stat.getCustomData() != null && stat.getCustomData().length() == 8 ? stat.getCustomData() : null));
/*	    if (GameConstants.GMS) {
	        c.write(MaplePacketCreator.getFamiliarInfo(player));
	    }*/
        
    }

    public static final void ChangeChannel(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        ...
        final int chc = inPacket.decodeByte() + 1;
