...
public class BBSHandler {
    ...
    public static final void BBSOperation(final InPacket inPacket, final MapleClient c) {
		...
		if (!c.getPlayer().getCheatTracker().canBBS()) {
		    ...
		    return;
                ...
                final boolean bEdit = inPacket.decodeByte() > 0;
                if (bEdit) {
                    localthreadid = inPacket.decodeInt();
                }
                final boolean bNotice = inPacket.decodeByte() > 0;
                final String title = correctLength(inPacket.decodeString(), 25);
                String text = correctLength(inPacket.decodeString(), 600);
                final int icon = inPacket.decodeInt();
		...
		if (!c.getPlayer().getCheatTracker().canBBS()) {
		    ...
		    return;
                ...
                localthreadid = inPacket.decodeInt();
                text = correctLength(inPacket.decodeString(), 25);
    ...
    private static void listBBSThreads(MapleClient c, int start) {
        ...
        c.write(MaplePacketCreator.BBSThreadList(World.Guild.getBBS(c.getPlayer().getGuildId()), start));
    }
        ...
        if (bbsList != null) {
            for (MapleBBSThread t : bbsList) {
                if (t != null && t.localthreadID == localthreadid) {
                    c.write(MaplePacketCreator.showThread(t));
                }
            }
        }
    }
