...
public class SummonHandler {
    ...
    public static final void MoveDragon(final InPacket inPacket, final MapleCharacter chr) {
        final Point oldPos = inPacket.decodePosition();
        final Point oldVPos = inPacket.decodePosition();
        final List<ILifeMovementFragment> res = MovementParse.parseMovement(inPacket, oldPos, MovementKind.DRAGON_MOVEMENT);
        if (chr != null && chr.getDragon() != null && res.size() > 0) {
            MovementParse.updatePosition(res, chr.getDragon(), 0);
            if (!chr.isHidden()) {
                chr.getMap().broadcastMessage(chr, MaplePacketCreator.moveDragon(chr.getDragon(), oldPos, oldVPos, res), chr.getTruePosition());
            }
                        ...
                        public void run() {
                            try {
                                if (clone.getMap() == map && clone.getDragon() != null) {
                                    ...
                                    if (!clone.isHidden()) {
                                        map.broadcastMessage(clone, MaplePacketCreator.moveDragon(clone.getDragon(), oldPos, oldVPos, res), clone.getTruePosition());
                                    }
    ...
    public static final void MoveSummon(final InPacket inPacket, final MapleCharacter chr) {
        if (chr == null || chr.getMap() == null) {
            return;
        }
        final MapleMapObject obj = chr.getMap().getMapObject(inPacket.decodeInt(), MapleMapObjectType.SUMMON);
        if (obj == null) {
            return;
        }
        if (obj instanceof MapleDragon) {
            MoveDragon(inPacket, chr);
            return;
        }
        ...
        final Point oldPos = inPacket.decodePosition();
        final Point oldVPos = inPacket.decodePosition();
        final List<ILifeMovementFragment> res = MovementParse.parseMovement(inPacket, oldPos, MovementKind.SUMMON_MOVEMENT);
        MovementParse.updatePosition(res, sum, 0);
        if (res.size() > 0) {
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.moveSummon(chr.getId(), sum.getObjectId(), oldPos, oldVPos, res), sum.getTruePosition());
        }
    }

    public static final void DamageSummon(final InPacket inPacket, final MapleCharacter chr) {
        final int unkByte = inPacket.decodeByte();
        final int damage = inPacket.decodeInt();
        final int monsterIdFrom = inPacket.decodeInt();
        //       inPacket.decodeByte(); // stance
    ...
    public static void SummonAttack(final InPacket inPacket, final MapleClient c, final MapleCharacter chr) {
        if (chr == null || !chr.isAlive() || chr.getMap() == null) {
            return;
        }
        final MapleMap map = chr.getMap();
        final MapleMapObject obj = map.getMapObject(inPacket.decodeInt(), MapleMapObjectType.SUMMON);
        ...
        inPacket.skip(8);
        final byte animation = inPacket.decodeByte();
        inPacket.skip(8);
        final byte numAttacked = inPacket.decodeByte();
        ...
        inPacket.skip(summon.getSkill() == 35111002 ? 24 : 12); //some pos stuff
        final List<Pair<Integer, Integer>> allDamage = new ArrayList<Pair<Integer, Integer>>();
        for (int i = 0; i < numAttacked; i++) {
            final MapleMonster mob = map.getMonsterByOid(inPacket.decodeInt());

            if (mob == null) {
                continue;
            }
            inPacket.skip(18); // who knows
            final int damage = inPacket.decodeInt();
        ...
        for (Pair<Integer, Integer> attackEntry : allDamage) {
            ...
            if (chr.isGM() || toDamage < (chr.getStat().getCurrentMaxBaseDamage() * 5.0 * (summonEffect.getSelfDestruction() + summonEffect.getDamage() + chr.getStat().getDamageIncrease(summonEffect.getSourceId())) / 100.0)) { //10 x dmg.. eh
                ...
                if (!mob.isAlive()) {
                    chr.getClient().write(MobPacket.killMonster(mob.getObjectId(), 1));
                }
            } else {
...
//            chr.removeVisibleMapObject(summon);
    ...
    public static final void RemoveSummon(final InPacket inPacket, final MapleClient c) {
        final MapleMapObject obj = c.getPlayer().getMap().getMapObject(inPacket.decodeInt(), MapleMapObjectType.SUMMON);
        if (obj == null || !(obj instanceof MapleSummon)) {
            return;
        }
        final MapleSummon summon = (MapleSummon) obj;
...
//        c.getPlayer().removeVisibleMapObject(summon);
    ...
    public static final void SubSummon(final InPacket inPacket, final MapleCharacter chr) {
        final MapleMapObject obj = chr.getMap().getMapObject(inPacket.decodeInt(), MapleMapObjectType.SUMMON);
        if (obj == null || !(obj instanceof MapleSummon)) {
            return;
        }
        ...
        switch (sum.getSkill()) {
            ...
            case 35111011: //healing
                ...
                chr.getClient().write(MaplePacketCreator.showOwnBuffEffect(sum.getSkill(), 2, chr.getLevel(), sum.getSkillLevel()));
            ...
            case 1321007: //beholder
                Skill bHealing = SkillFactory.getSkill(inPacket.decodeInt());
                ...
                chr.getClient().write(MaplePacketCreator.showOwnBuffEffect(sum.getSkill(), 2, chr.getLevel(), bHealingLvl));
        ...
        if (GameConstants.isAngel(sum.getSkill())) {
            ...
            chr.getClient().write(MaplePacketCreator.showOwnBuffEffect(sum.getSkill(), 2, 2, 1));
            chr.getMap().broadcastMessage(chr, MaplePacketCreator.showBuffeffect(chr.getId(), sum.getSkill(), 2, 2, 1), false);
        }
    }

    public static final void SummonPVP(final InPacket inPacket, final MapleClient c) {
        ...
        int tick = -1;
        if (inPacket.getUnreadAmount() == 27) {
            inPacket.skip(23);
            tick = inPacket.decodeInt();
        }
        final MapleSummon summon = (MapleSummon) obj;
        if (summon.getOwnerId() != chr.getId() || summon.getSkillLevel() <= 0) {
            chr.dropMessage(5, "Error.");
