...
public class FamilyHandler {
    ...
    public static final void RequestFamily(final InPacket inPacket, MapleClient c) {
        MapleCharacter chr = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.decodeString());
        if (chr != null) {
            c.write(FamilyPacket.getFamilyPedigree(chr));
        }
    }

    public static final void OpenFamily(final InPacket inPacket, MapleClient c) {
        c.write(FamilyPacket.getFamilyInfo(c.getPlayer()));
    }

    public static final void UseFamily(final InPacket inPacket, MapleClient c) {
        ...
        switch (entry) {
            case Teleport: //teleport: need add check for if not a safe place
                victim = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.decodeString());
    ...
    public static final void FamilyOperation(final InPacket inPacket, MapleClient c) {
        if (c.getPlayer() == null) {
            return;
        }
        MapleCharacter addChr = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.decodeString());
        if (addChr == null) {
    ...
    public static final void FamilyPrecept(final InPacket inPacket, MapleClient c) {
        MapleFamily fam = World.Family.getFamily(c.getPlayer().getFamilyId());
        if (fam == null || fam.getLeaderId() != c.getPlayer().getId()) {
            return;
        }
        fam.setNotice(inPacket.decodeString());
    }

    public static final void FamilySummon(final InPacket inPacket, MapleClient c) {
        MapleFamilyBuff cost = MapleFamilyBuff.Summon;
        MapleCharacter tt = c.getChannelServer().getPlayerStorage().getCharacterByName(inPacket.decodeString());
    ...
    public static final void DeleteJunior(final InPacket inPacket, MapleClient c) {
    ...
    public static final void DeleteSenior(final InPacket inPacket, MapleClient c) {
    ...
    public static final void AcceptFamily(InPacket inPacket, MapleClient c) {
        MapleCharacter inviter = c.getPlayer().getMap().getCharacterById(inPacket.decodeInt());
        if (inviter != null && c.getPlayer().getSeniorId() == 0 && (c.getPlayer().isGM() || !inviter.isHidden()) && inviter.getLevel() - 20 <= c.getPlayer().getLevel() && inviter.getLevel() >= 10 && inviter.getName().equals(inPacket.decodeString()) && inviter.getNoJuniors() < 2 /*&& inviter.getFamily().getGens() < 1000*/ && c.getPlayer().getLevel() >= 10) {
            boolean accepted = inPacket.decodeByte() > 0;
