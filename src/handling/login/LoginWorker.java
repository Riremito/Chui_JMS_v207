...
public class LoginWorker {
    ...
    public static void registerClient(final MapleClient c) {
        if (LoginServer.isAdminOnly() && !c.isGm() && !c.isLocalhost()) {
            c.write(MaplePacketCreator.serverNotice(1, "The server is currently set to Admin login only.\r\nWe are currently testing some issues.\r\nPlease try again later."));
        ...
        if (c.finishLogin() == 0) {
            ...
            c.write(LoginPacket.LoginDetail(1));
...
/*        if (!ServerConstants.isEligible(c.getSessionIPAddress())) {
            try {
                ...
                ps.setString(2, c.getRemoteAddress().toString());
